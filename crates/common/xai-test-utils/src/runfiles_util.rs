...
// Modified by the Bot project on 2026-09-15: support Cargo all-feature builds.
...
pub fn try_resolve_runfiles(_path: &str) -> Option<PathBuf> {
    #[cfg(feature = "bazel")]
    {
        ...
        let path = match option_env!("REPOSITORY_NAME") {
            Some(repository) => r.rlocation_from(_path, repository),
            None => r.rlocation(_path),
        }?;
