// Modified by the Bot project on 2026-09-15: use the shared TLS client in trace tests.
...
mod tests {
    ...
    async fn traced_client_injects_client_span_not_parent_on_wire() {
        ...
        let client = traced_client(
            xai_grok_extra_ca::build_reqwest_client(|builder| builder).expect("trace test client"),
        );
