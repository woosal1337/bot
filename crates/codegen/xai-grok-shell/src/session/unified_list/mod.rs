// Modified by the Bot project on 2026-09-15: keep imported lane tests lint-clean.
mod cursor;
mod envelope;
mod facets;
mod row;
...
mod tests {
    ...
    #[test]
    #[serial_test::serial]
    fn parse_list_req_ignores_disabled_process_chat_mode() {
        use crate::agent::chat_modes::GROK_CHAT_MODE_ENV;
        ...
        }
        {
            ...
            assert_eq!(
                parsed.facet_filters.get(KIND_FACET_KEY),
                Some(&vec![serde_json::json!("build")]),
                "disabled process chat mode must preserve the client kind"
            );
            assert_eq!(
                parsed.facet_filters.get("starred"),
                Some(&vec![serde_json::json!(true)]),
            ...
            assert_eq!(
                parsed.facet_filters.get(KIND_FACET_KEY),
                None,
                "disabled process chat mode must not add a kind"
            );
            for (input, expected) in [
                ...
                let req = parse_list_req(&input.to_string()).expect("parse");
                let parsed = ParsedMeta::parse(req.meta.as_ref());
                assert_eq!(
                    parsed.facet_filters.get(KIND_FACET_KEY),
                    Some(&expected),
                    "disabled process chat mode must preserve kind: {input}"
                );
            }
        }
    }
