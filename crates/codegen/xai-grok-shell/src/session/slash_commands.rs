// Modified by the Bot project on 2026-09-14: reserve provider, account, and permission commands and remove privacy.
...
fn workflow_meta(
    workflow: &crate::session::workflow::registry::WorkflowListing,
) -> Option<acp::Meta> {
    let ownership =
        CommandOwnership::current("grok", workflow.source, CommandKind::Workflow).ok()?;
    let mut meta = acp::Meta::new();
    meta.insert(
        "botOwnership".to_string(),
...
pub(super) fn available_commands(
    ...
    commands.extend(catalog.builtins.iter().map(|builtin| {
        acp::AvailableCommand::new(builtin.name.to_string(), builtin.description.to_string())
            ...
            .meta(exact_workflow_projection(builtin, workflows).and_then(workflow_meta))
    }));
    ...
    commands.extend(catalog.workflows.iter().filter_map(|workflow| {
        let meta = workflow_meta(workflow)?;
        Some(
            acp::AvailableCommand::new(
                workflow.name.clone(),
