...
#[tokio::test(flavor = "current_thread")]
async fn rewind_waits_for_image_strip_before_commit() {
    let local = tokio::task::LocalSet::new();
    local
        .run_until(async {
            let (gateway_tx, _) =
            ...
            let terminal_request = timed_out.clone();
            let terminal = tokio::task::spawn_local(async move {
                terminal_actor
                    .handle_sampling_event(completed_event(&terminal_request))
                    .await;
            });
            tokio::task::yield_now().await;
            assert!(
            ...
            terminal.await.expect("terminal event completes");
            let rewind = rewind
                .await
...
async fn rejected_rewind_preserves_queued_image_strip() {
    ...
    local
        .run_until(async {
            ...
            let terminal_actor = Arc::clone(&actor);
            let terminal_request = request_id.clone();
            let terminal = tokio::task::spawn_local(async move {
                terminal_actor
                    .handle_sampling_event(completed_event(&terminal_request))
                    .await;
...
async fn pending_strip_bound_preserves_detached_and_new_url_entries() {
    ...
    local
        .run_until(async {
            ...
            let strip_blocker = actor.image_strip_rewrite_barrier.lock_strip().await;
            let terminal_actor = Arc::clone(&actor);
            let terminal_request = applying_id.clone();
            let terminal = tokio::task::spawn_local(async move {
                terminal_actor
                    .handle_sampling_event(completed_event(&terminal_request))
                    .await;
