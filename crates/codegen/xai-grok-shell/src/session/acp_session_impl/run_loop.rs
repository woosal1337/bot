// Modified by the Bot project on 2026-09-12: hook action capability metadata.
...
async fn finish_session_exit(session: &SessionActor, timer: &SharedSessionEndTimer) {
    if !session.startup_hints.is_subagent {
        let _tasks = session_end::timed(timer, Phase::BackgroundTasksSave);
        ...
        tokio::select! {
                ...
                maybe_cmd = cmd_rx.recv() => {
                    let Some(cmd) = maybe_cmd else {
                        ...
                        finish_session_exit(&session, &end_timer).await;
