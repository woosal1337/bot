...
impl SessionActor {
    ...
    /// Shared round-completion bookkeeping for plan cleanup, signal tracking, and token sums.
    /// Runs identically for the native and StructuredOutput-tool completion paths.
    /// The turn-end snapshot is taken once per turn after the last round.
    ...
    pub(super) fn persist_turn_snapshot(&self, snapshot: Option<&TurnDeltaSnapshot>) {
