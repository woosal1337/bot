// Modified by the Bot project on 2026-09-13: Removed updater-only inspection fields.
...
pub(crate) struct EnforcedPolicy {
    ...
    /// (requirements layer for alwaysApprove or managed-settings.json for
    /// telemetry), or the diskless macOS MDM source id.
    pub source: String,
}
...
fn permission_policy_report(
    ...
    if let Some(src) = &ms.features.source_path {
        ...
        for (flag, setting) in [(ms.features.disable_telemetry, EnforcedSetting::Telemetry)] {
            if flag == Some(true) {
                enforced.push(EnforcedPolicy {
                    setting,
                    enabled: false,
...
mod tests {
    ...
    fn claude_bypass_lock_is_advisory_not_enforced() {
        ...
        } = permission_policy_report(&managed_features(Some(true), None), None);
        assert!(advisory);
        assert!(
            enforced.is_empty(),
        ...
        // Managed telemetry clamps remain enforced. The removed feedback setting is ignored.
        let PermissionPolicyReport {
            enforced,
            claude_bypass_lock_advisory: advisory,
        } = permission_policy_report(&managed_features(Some(true), Some(true)), None);
        assert!(advisory);
    ...
    fn requirements_lock_reports_always_approve_enforced() {
        ...
        let PermissionPolicyReport {
            enforced,
            claude_bypass_lock_advisory: advisory,
        } = permission_policy_report(&managed_features(Some(true), None), Some(&lock));
        assert!(advisory);
        assert_eq!(enforced.len(), 1);
        assert_eq!(enforced[0].source, "/etc/grok/requirements.toml");
    }
