...
//! Send-safe view of the agent's in-flight work, shared with the leader's `RelaunchForUpdate` drain.
...
/// Default bound on a process-exit session flush ([`AgentActivity::flush_all_sessions`]). Leader version handoff and the in-process agent's `/exit` / headless-quit path both use it.
...
impl AgentActivity {
    ...
    /// Purging happens only here, so a dead entry can linger until the next register.
