// Modified by the Bot project on 2026-09-12: remove privacy banner settings notifications.
...
pub struct MvpAgent {
    /// LEADER-SAFE(shared): `Send + Sync` mirror of per-session activity for leader shutdown, which cannot read the `!Send` maps.
