// Modified by the Bot project on 2026-09-13: xAI feedback endpoint removal.
use super::*;
...
#[tokio::test(flavor = "current_thread")]
async fn removed_feedback_extensions_are_not_routable() {
    use acp::Agent as _;
    let agent = build_minimal_agent_for_tests();
    for method in [
        "x.ai/feedback",
        ...
        let error = agent
            ...
            .await
            .expect_err("removed feedback extension must be unavailable");
        assert_eq!(error.code, acp::Error::method_not_found().code, "{method}");
    }
}

#[tokio::test(flavor = "current_thread")]
async fn removed_product_service_extensions_are_not_routable() {
    use acp::Agent as _;
    let agent = build_minimal_agent_for_tests();
    for method in [
        "x.ai/rollout/survey",
        ...
        let error = agent
            .ext_method(acp::ExtRequest::new(method, params.into()))
            .await
            .expect_err("removed product service extension must be unavailable");
        assert_eq!(error.code, acp::Error::method_not_found().code, "{method}");
    }
}

#[tokio::test(flavor = "current_thread")]
async fn auth_response_omits_provider_access_gate() {
    let agent = build_minimal_agent_for_tests();
    agent
        .auth_manager
        .hot_swap(xai_grok_login::GrokAuth::test_default());
    agent.cfg.borrow_mut().remote_settings = Some(
        serde_json::from_value(serde_json::json!({
            "gate_message": "Subscribe to continue",
            "gate_url": "https://example.com/subscribe",
            "gate_label": "Subscribe",
            "allow_access": false
        }))
        .expect("legacy settings remain parseable"),
    );
    let response = agent.auth_response_with_meta();
    let meta = response.meta.expect("authenticated response has metadata");
    assert!(!meta.contains_key("gate"));
}
