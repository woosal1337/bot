// Modified by the Bot project on 2026-09-13: Removed provider upsell copy.
...
/// OAuth or session rate-limit copy.
pub const RATE_LIMITED_USER_MESSAGE_OAUTH: &str =
    "You\u{2019}ve hit the provider rate limit. Try again later.";

/// API-key rate-limit copy.
pub const RATE_LIMITED_USER_MESSAGE_API_KEY: &str = "You\u{2019}ve hit the API rate limit for this account. Check the provider account limits, or try again later.";

/// User-facing text for an ACP -32003 rate-limit error.
/// Provider subscription offers use the common rate-limit copy. Other server details pass through after the sampling error prefix is removed.
...
pub fn format_rate_limited_user_message(
    ...
    if let Some(detail) = server_detail.map(str::trim).filter(|s| !s.is_empty()) {
        ...
        if contains_subscription_offer(detail) {
            return rate_limit_fallback(is_api_key_auth).to_string();
        }
        return detail.to_string();
    }
    rate_limit_fallback(is_api_key_auth).to_string()
}

fn rate_limit_fallback(is_api_key_auth: bool) -> &'static str {
    if is_api_key_auth {
...
fn contains_subscription_offer(detail: &str) -> bool {
    let d = detail.to_ascii_lowercase();
    d.contains("subscription:free-usage-exhausted")
        || d.contains("grok.com/supergrok")
        || d.contains("upgrade to a grok subscription")
}
...
mod tests {
    ...
    fn rate_limited_fallback_oauth_vs_api_key() {
        ...
        assert!(RATE_LIMITED_USER_MESSAGE_API_KEY.contains("provider account limits"));
    }
    ...
    #[test]
    fn format_rate_limited_filters_subscription_offer() {
    ...
    #[test]
    fn subscription_offer_detection_covers_provider_copy() {
        assert!(contains_subscription_offer(
        ...
        assert!(contains_subscription_offer(
            "Upgrade to a Grok subscription"
        ));
        assert!(!contains_subscription_offer("throttled"));
        assert!(!contains_subscription_offer(
    ...
    #[test]
    fn format_rate_limited_free_usage_uses_common_copy() {
