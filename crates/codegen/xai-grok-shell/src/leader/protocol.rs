// Modified by the Bot project on 2026-09-13: Retained generic version handoff without the updater.
...
pub struct ClientCapabilities {
    ...
    /// The leader logs a warning when this differs from its own version.
    #[serde(default)]
...
pub struct LeaderCapabilities {
    ...
    /// Whether the leader supports [`ControlCommand::RelaunchForUpdate`], a disruptive relaunch onto a newer binary.
...
/// ## Runtime status | Variant | Emitted today? | Notes | |---------|---------------|-------| | `AutoUpdate` | **Yes** — during a binary-version handoff | | | `Manual` | **Yes** — default for SIGTERM, test cancellation, all other paths | | | `IdleTimeout` | **No** — reserved for a future idle-timeout feature | |
#[derive(Debug, Clone, PartialEq, Eq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum ShutdownReason {
    /// Leader is shutting down for a binary-version handoff.
