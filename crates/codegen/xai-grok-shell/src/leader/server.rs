// Modified by the Bot project on 2026-09-13: Removed stale updater references.
...
struct ClientState {
    ...
    /// Pre-registration connections (which may time out) must not inflate the count and block version handoffs.
    registered: bool,
}
#[derive(Debug, Clone)]
...
/// Then flush every session actor, set [`ShutdownReason::AutoUpdate`], and cancel.
...
/// A leader serving only interactive clients (TUI dashboard, IDE) thus never duplicates its ACP stream onto the relay. The [`ControlCommand::RelaunchForUpdate`] handler sends [`ShutdownReason::AutoUpdate`] before cancelling. Clients then see the real reason; senders must write before cancelling.
