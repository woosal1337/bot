...
/// For a **managed install** — the running binary lives under `grok_home` (e.g. `~/.grok/...`) — prefer the managed `~/.grok/bin/grok` symlink. After the symlink changes, `current_exe()` still resolves (via `/proc/self/exe` on Linux) to the old versioned target, so spawning it would relaunch the stale binary.
