...
#[test]
fn rejects_ultra_without_sending_a_different_effort() {
    assert!(crate::ReasoningEffort::Ultra.to_responses_api().is_err());
    assert_eq!(
        "ultra"
            .parse::<crate::ReasoningEffort>()
            .unwrap()
            .to_string(),
        "ultra"
    );
...
fn test_conversation_request_to_responses_api() {
    ...
    let responses_req: rs::CreateResponse = (&req).try_into().unwrap();
...
fn test_malformed_tool_arguments_sanitized_in_responses_api() {
    ...
    let rs_req: crate::rs::CreateResponse = (&req).try_into().unwrap();
...
fn test_responses_request_carries_reasoning_effort_nested() {
    for (variant, expected) in [
        ...
        let resp: crate::rs::CreateResponse = (&req).try_into().unwrap();
...
fn test_btw_cross_api_responses_no_regressions() {
    ...
    let resp: rs::CreateResponse = (&req).try_into().unwrap();
    let json = serde_json::to_value(&resp).unwrap();
...
fn serialized_body_contains_no_placeholder_strings() {
    ...
    let cr: rs::CreateResponse = (&req).try_into().unwrap();
