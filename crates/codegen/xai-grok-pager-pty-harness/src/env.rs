...
fn local_pager_binary_path() -> Result<PathBuf> {
    Ok(target_dir()?
        .join("debug")
        .join(format!("bot{}", std::env::consts::EXE_SUFFIX)))
}
...
fn ensure_local_pager_binary(binary: &std::path::Path) -> Result<()> {
    ...
    cmd.current_dir(workspace_root()?)
        .args(["build", "-p", "xai-grok-pager-bin", "--bin", "bot"])
        .stdin(Stdio::null())
...
/// `PAGER_BINARY`, then `CARGO_BIN_EXE_bot`, else build `xai-grok-pager-bin` (the package that owns the binary).
pub fn pager_binary() -> Result<PathBuf> {
    ...
    if let Ok(path) = std::env::var("CARGO_BIN_EXE_bot") {
        let p = PathBuf::from(path);
        if p.exists() {
            return Ok(p);
        }
