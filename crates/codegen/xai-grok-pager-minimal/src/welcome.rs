// Modified by the Bot project on 2026-09-11: product welcome identity.
...
//! The card holds the block logo, the version, the cwd, the model, and a one-line hint.
...
pub fn maybe_commit_welcome(app: &mut AppView, terminal: &mut PagerTerminal) {
    ...
    let logo_lines = if width.saturating_sub(2) >= minimal_api::compact_logo_visual_width() {
        minimal_api::compact_logo_line_count()
    } else {
        0
    };
