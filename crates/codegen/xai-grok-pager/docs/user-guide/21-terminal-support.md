...
behavior, and clipboard routes. The in-app command can also check live session details such as
...
`grok doctor --json` reports the same color capability when piped.
...
Run `/doctor` to inspect the local installation and provider connections.
