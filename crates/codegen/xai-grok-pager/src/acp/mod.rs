// Modified by the Bot project on 2026-09-11: provider-neutral agent connections.
...
mod codex;
pub(crate) use codex::spawn_codex;
...
impl AgentEndpoint {
    pub(crate) fn into_headless(self) -> (AcpAgentTx, AcpClientRx, spawn::AgentShutdownGuard) {
        let thread = match self.location {
            AgentLocation::Thread(thread) => Some(thread),
            AgentLocation::Leader(_) => None,
        };
        let guard = spawn::AgentShutdownGuard::new(self.cancel, thread);
        (self.tx, self.rx, guard)
    }
}
...
pub async fn connect(cancel: &CancellationToken, flags: ConnectFlags) -> Result<AcpConnection> {
    ...
    initialize_connection(AgentEndpoint::from(spawned), &flags).await
}
pub async fn connect_codex(
    ...
    let spawned = codex::spawn_codex(executable, cancel, &flags).await?;
    initialize_connection(spawned, &flags).await
}
...
pub async fn connect_via_leader(
    ...
    initialize_connection(endpoint, &flags).await
}
...
pub fn startup_auth_metadata(
    ...
    let needs_login = first_method
        .map(auth_method_is_interactive)
        .unwrap_or(false);
    if !needs_login {
...
/// Uses known interactive IDs or the provider's `external_provider` metadata.
pub fn find_interactive_login_method(
    auth_methods: &[acp::AuthMethod],
) -> (Option<String>, Option<acp::AuthMethodId>, AuthStartMode) {
    let interactive = auth_methods.iter().find(|m| auth_method_is_interactive(m));
    match interactive {
        Some(method) => {
            let is_provider = method
                .meta()
...
fn auth_method_is_interactive(method: &acp::AuthMethod) -> bool {
    AuthMethodKind::from_id(method.id()).needs_interactive_login()
        || method
            .meta()
            .as_ref()
...
mod tests {
    ...
    #[test]
    fn startup_auth_accepts_an_external_provider_method() {
        let meta = serde_json::json!({
        ...
        let (needs, label, method_id, mode) = startup_auth_metadata(&methods);
        assert!(needs);
        assert_eq!(label.as_deref(), Some("Sign in with ChatGPT"));
        assert_eq!(
            method_id.as_ref().map(|id| id.0.as_ref()),
            Some("codex.chatgpt")
        );
        assert_eq!(mode, AuthStartMode::Command);
    }
