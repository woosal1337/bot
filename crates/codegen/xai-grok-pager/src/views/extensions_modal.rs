// Modified by the Bot project on 2026-09-13: provider-native capabilities and extension recovery.
...
impl ExtensionsTab {
    ...
    const CODEX: &[Self] = &[Self::Hooks, Self::Plugins, Self::Skills, Self::McpServers];
    const NONE: &[Self] = &[];

    pub fn for_provider(provider: &crate::provider::ProviderId) -> &'static [Self] {
        match provider {
            crate::provider::ProviderId::Grok => Self::ALL,
            crate::provider::ProviderId::Codex => Self::CODEX,
            crate::provider::ProviderId::Gemini
            | crate::provider::ProviderId::Claude
            | crate::provider::ProviderId::Custom(_) => Self::NONE,
        }
    }

    pub fn available() -> &'static [Self] {
        Self::for_provider(&crate::provider::active_provider())
    }
    ...
    fn load_error_title(self) -> &'static str {
        match self {
            Self::Hooks => "Could not load hooks.",
            Self::Plugins => "Could not load plugins.",
            Self::Marketplace => "Could not load the marketplace.",
            Self::Skills => "Could not load skills.",
            Self::Workflows => "Could not load workflows.",
            Self::McpServers => "Could not load MCP servers.",
        }
    }

    fn retry_label(self) -> &'static str {
        match self {
            Self::Marketplace | Self::McpServers => "r refresh",
            Self::Hooks | Self::Plugins | Self::Skills | Self::Workflows => "r reload",
        }
    }
...
struct ExtensionLoadErrorRow {
    label: String,
    right_label: String,
    detail_lines: Vec<String>,
}

fn extension_load_error_row(tab: ExtensionsTab, message: &str) -> ExtensionLoadErrorRow {
    let detail = message
        .lines()
        .map(str::trim)
        ...
        .collect::<Vec<_>>()
        .join(" ");
    ExtensionLoadErrorRow {
        label: tab.load_error_title().to_owned(),
        right_label: tab.retry_label().to_owned(),
        detail_lines: vec![if detail.is_empty() {
            "The provider returned no error details.".to_owned()
        } else {
            detail
        }],
    }
...
pub fn available_extensions_action_keys(state: &ExtensionsModalState) -> Vec<(char, &'static str)> {
    let mut keys = if state.active_tab == ExtensionsTab::Hooks
        && matches!(
            state.hooks_data,
            TabDataState::Loaded(ref data) if !data.actions_supported
        ) {
        vec![('r', "reload")]
    } else {
        extensions_action_keys(state.active_tab)
    };
    if state.active_tab == ExtensionsTab::Plugins && !state.capabilities.plugin_updates {
        keys.retain(|(key, _)| *key != 'u');
    }
    if state.active_tab == ExtensionsTab::McpServers && !state.capabilities.mcp_mutations {
        keys.retain(|(key, _)| *key == MCP_SERVERS_REFRESH_KEY);
    }
    keys
}
...
pub struct ExtensionsModalState {
    ...
    pub tabs: &'static [ExtensionsTab],
    pub capabilities: crate::provider::ExtensionCapabilities,
    /// Currently active tab (source of truth). `window.active_tab` (a `usize` index) is derived from
    /// this in the render path via `tabs.position()`. Only this field should be mutated
...
impl ExtensionsModalState {
    pub fn new(tab: ExtensionsTab) -> Self {
        Self::new_for_provider(tab, &crate::provider::active_provider())
    }

    pub fn new_for_provider(tab: ExtensionsTab, provider: &crate::provider::ProviderId) -> Self {
        let tabs = ExtensionsTab::for_provider(provider);
        let active_tab = if tabs.contains(&tab) {
            tab
        } else {
            tabs.first().copied().unwrap_or(tab)
        };
        Self {
            window: ModalWindowState::with_tabs(tabs.len()),
            tabs,
            capabilities: crate::provider::extension_capabilities(provider),
            active_tab,
    ...
    pub fn fail_load(&mut self, tab: ExtensionsTab, message: String) -> bool {
        match tab {
            ExtensionsTab::Hooks if matches!(self.hooks_data, TabDataState::Loading) => {
                self.hooks_data = TabDataState::Error(message);
                true
            }
            ExtensionsTab::Plugins if matches!(self.plugins_data, TabDataState::Loading) => {
                self.plugins_data = TabDataState::Error(message);
                true
            }
            ExtensionsTab::Marketplace
                if matches!(self.marketplace_data, TabDataState::Loading) =>
            {
                self.marketplace_data = TabDataState::Error(message);
                true
            }
            ExtensionsTab::Skills if matches!(self.skills_data, TabDataState::Loading) => {
                self.skills_data = TabDataState::Error(message);
                true
            }
            ExtensionsTab::Workflows if matches!(self.workflows_data, TabDataState::Loading) => {
                self.workflows_data = TabDataState::Error(message);
                true
            }
            ExtensionsTab::McpServers if matches!(self.mcps_data, TabDataState::Loading) => {
                self.mcps_data = TabDataState::Error(message);
                true
            }
            _ => false,
        }
    ...
    pub fn mcp_auth_intercept_on_expand(&self) -> bool {
        if !self.capabilities.mcp_mutations || self.active_tab != ExtensionsTab::McpServers {
            return false;
        }
...
pub fn render_extensions_modal(
        ...
        match state.active_tab {
            ExtensionsTab::Skills => {
                if let TabDataState::Loaded(ref skills) = state.skills_data {
                    ...
                    let row = extension_load_error_row(ExtensionsTab::Skills, msg);
                    entry_labels.push(row.label);
                    entry_right_labels.push(row.right_label);
                    entry_desc_lines.push(row.detail_lines.clone());
                    entry_summary_lines.push(row.detail_lines);
                    entry_fields.push(vec![]);
                    entry_is_header.push(false);
                    entry_dimmed.push(false);
                    entry_indent.push(0);
            ...
            ExtensionsTab::Workflows => {
                    ...
                    build_workflows_picker_rows(&state.workflows_data, state.picker_state.query())
                {
                    ...
                    entry_summary_lines.push(row.summary_lines);
            ...
            ExtensionsTab::Plugins => {
                if let TabDataState::Loaded(ref response) = state.plugins_data {
                    ...
                    let row = extension_load_error_row(ExtensionsTab::Plugins, msg);
                    entry_labels.push(row.label);
                    entry_right_labels.push(row.right_label);
                    entry_desc_lines.push(row.detail_lines.clone());
                    entry_summary_lines.push(row.detail_lines);
            ...
            ExtensionsTab::Hooks => {
                if let TabDataState::Loaded(ref data) = state.hooks_data {
                    if !data.actions_supported {
                        entry_labels.push("Provider-managed hooks".into());
                        entry_right_labels.push("[read only]".into());
                        entry_desc_lines.push(vec![
                            "Edit hooks with the provider's native configuration.".into(),
                        ]);
                        entry_summary_lines.push(vec![]);
                        entry_fields.push(vec![]);
                    ...
                    let row = extension_load_error_row(ExtensionsTab::Hooks, msg);
                    entry_labels.push(row.label);
                    entry_right_labels.push(row.right_label);
                    entry_desc_lines.push(row.detail_lines.clone());
                    entry_summary_lines.push(row.detail_lines);
            ...
            ExtensionsTab::Marketplace => {
                if let TabDataState::Loaded(ref data) = state.marketplace_data {
                    ...
                    let row = extension_load_error_row(ExtensionsTab::Marketplace, msg);
                    entry_labels.push(row.label);
                    entry_right_labels.push(row.right_label);
                    entry_desc_lines.push(row.detail_lines.clone());
                    entry_summary_lines.push(row.detail_lines);
            ...
            ExtensionsTab::McpServers => {
                if let TabDataState::Loaded(ref servers) = state.mcps_data {
                    ...
                    let row = extension_load_error_row(ExtensionsTab::McpServers, msg);
                    entry_labels.push(row.label);
                    entry_right_labels.push(row.right_label);
                    entry_desc_lines.push(row.detail_lines.clone());
                    entry_summary_lines.push(row.detail_lines);
    ...
    let action_keys = available_extensions_action_keys(state);
    ...
    // correctly in the default Shortcut renderer.
    ...
    } else if state.picker_state.search_active && state.input.is_none() && state.mcp_setup.is_none()
    {
        ...
        let i_is_action_key = available_extensions_action_keys(state)
            .iter()
            .any(|&(ch, _)| ch == 'i');
        if !i_is_action_key {
...
/// Which full-window message overlay owns the footer.
...
mod tests {
    ...
    #[test]
    fn read_only_hooks_only_offer_reload() {
        ...
        insta::assert_debug_snapshot!(available_extensions_action_keys(&state), @r###"
        [
            (
                'r',
                "reload",
    ...
    fn mcp_collapsed_managed_section_omits_server_rows() {
        ...
        assert!(
            rows.labels
                .iter()
                .any(|l| l.starts_with("Managed by provider")),
    ...
    #[test]
    fn workflows_tab_error_renders_recovery_row() {
        let mut state = ExtensionsModalState::new(ExtensionsTab::Workflows);
        state.workflows_data = TabDataState::Error("boom".into());
        let area = Rect::new(0, 0, 100, 40);
        let mut buf = Buffer::empty(area);
        render_extensions_modal(&mut buf, area, &mut state, None, false, 0);
        assert_eq!(buffer_count(&buf, "Could not load workflows."), 1);
        assert_eq!(buffer_count(&buf, "r reload"), 2);
        assert_eq!(buffer_count(&buf, "boom"), 1);
    }

    #[test]
    fn hooks_tab_error_renders_recovery_row() {
        let mut state = ExtensionsModalState::new(ExtensionsTab::Hooks);
        state.hooks_data = TabDataState::Error("Method not found".into());
        let area = Rect::new(0, 0, 100, 40);
        let mut buf = Buffer::empty(area);
        render_extensions_modal(&mut buf, area, &mut state, None, false, 0);
        assert_eq!(buffer_count(&buf, "Could not load hooks."), 1);
        assert_eq!(buffer_count(&buf, "r reload"), 2);
        assert_eq!(buffer_count(&buf, "Method not found"), 1);
    }

    #[test]
    fn extension_load_errors_use_actionable_provider_neutral_copy() {
        let rows = ExtensionsTab::ALL
            .iter()
            .map(|tab| extension_load_error_row(*tab, "Method not found\nTry again."))
            .collect::<Vec<_>>();
        insta::assert_debug_snapshot!(rows, @r###"
        [
            ExtensionLoadErrorRow {
                label: "Could not load hooks.",
                right_label: "r reload",
                detail_lines: [
                    "Method not found Try again.",
                ],
            },
            ExtensionLoadErrorRow {
                label: "Could not load plugins.",
                right_label: "r reload",
                detail_lines: [
                    "Method not found Try again.",
                ],
            ...
            ExtensionLoadErrorRow {
                label: "Could not load the marketplace.",
                right_label: "r refresh",
                detail_lines: [
                    "Method not found Try again.",
                ],
            },
            ...
            ExtensionLoadErrorRow {
                label: "Could not load workflows.",
                right_label: "r reload",
                detail_lines: [
                    "Method not found Try again.",
                ],
            ...
            ExtensionLoadErrorRow {
                label: "Could not load MCP servers.",
                right_label: "r refresh",
                detail_lines: [
                    "Method not found Try again.",
                ],
    ...
    #[test]
    fn codex_uses_only_supported_extension_tabs_and_actions() {
        let state = ExtensionsModalState::new_for_provider(
            ExtensionsTab::Marketplace,
        ...
        let mut plugins = ExtensionsModalState::new_for_provider(
            ExtensionsTab::Plugins,
            &crate::provider::ProviderId::Codex,
        );
        assert_eq!(
            available_extensions_action_keys(&plugins),
            vec![
                ('r', "reload"),
                ('a', "install"),
                (' ', "toggle"),
        ...
        plugins.switch_tab(ExtensionsTab::McpServers);
        assert_eq!(
            available_extensions_action_keys(&plugins),
            vec![(MCP_SERVERS_REFRESH_KEY, "refresh")]
        );
        plugins.switch_tab(ExtensionsTab::Workflows);
        assert_eq!(plugins.active_tab, ExtensionsTab::McpServers);
    }
