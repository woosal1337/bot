// Modified by the Bot project on 2026-09-13: Removed xAI upsell question types.
...
pub use xai_grok_tools::implementations::grok_build::ask_user_question::{
    AskUserQuestionMode, Question, QuestionMetadata, QuestionOption,
};
...
pub struct QuestionViewState {
    ...
    pub question_metadata: HashMap<String, QuestionMetadata>,
    pub auto_resolve_at: Option<Instant>,
}
...
impl QuestionViewState {
    ...
    pub fn with_provider_behavior(
        mut self,
        question_metadata: HashMap<String, QuestionMetadata>,
        auto_resolution_ms: Option<u64>,
    ) -> Self {
        self.question_metadata = question_metadata;
        self.auto_resolve_at = auto_resolution_ms.and_then(|milliseconds| {
            self.opened_at
    ...
    pub fn allows_freeform(&self, question_idx: usize) -> bool {
        if self.no_freeform {
            return false;
        }
        self.questions
            ...
            .and_then(|id| self.question_metadata.get(id))
            .is_none_or(|metadata| metadata.allow_freeform)
    }

    pub fn active_question_is_secret(&self) -> bool {
        self.questions
            .get(self.active_tab)
            .and_then(|question| question.id.as_ref())
            .and_then(|id| self.question_metadata.get(id))
            .is_some_and(|metadata| metadata.secret)
    }

    pub fn auto_resolution_pending(&self) -> bool {
        self.auto_resolve_at.is_some()
    }

    pub fn auto_resolution_due(&self, now: Instant) -> bool {
        self.auto_resolve_at.is_some_and(|deadline| now >= deadline)
    }

    pub fn snooze_auto_resolution(&mut self) {
    ...
    pub fn total_items(&self, question_idx: usize) -> usize {
        let freeform = usize::from(self.allows_freeform(question_idx));
        self.questions
            .get(question_idx)
    ...
    pub fn is_on_freeform_row(&self) -> bool {
        if !self.allows_freeform(self.active_tab) {
            return false;
        }
...
impl QuestionViewState {
    ...
    /// Activate freeform input for the active question. Fixed-choice questions have no freeform row.
...
pub fn render_question_view(
    ...
    let sticky_freeform = !is_input_mode && state.allows_freeform(q_idx);
    ...
    if sticky_freeform {
        ...
        if freeform_y >= y {
            ...
            let masked_freeform = state.active_question_is_secret().then_some("******");
            let freeform_line = build_freeform_line(
                freeform_idx == cursor,
                hovered_item == Some(freeform_idx),
                masked_freeform.unwrap_or(freeform_text),
                freeform_selected,
                is_multi,
                theme,
                focused,
...
mod tests {
    ...
    /// `no_freeform` questions have no "Other" row, so activating freeform input must be impossible.
    /// Focus stays in Navigation and nothing gets marked selected.
    /// Regression test for fixed-choice questions accepting text below the last option.
    #[test]
    ...
    #[test]
    fn provider_metadata_controls_each_freeform_row() {
        let questions = vec![
            Question {
                question: "Fixed".into(),
                options: vec![QuestionOption {
        ...
        let mut state = QuestionViewState::new("tc".into(), questions, StashedPrompt::default())
            .with_provider_behavior(metadata, None);
        assert!(!state.allows_freeform(0));
        assert_eq!(state.total_items(0), 1);
        state.next_question();
        assert!(state.allows_freeform(1));
        assert!(state.active_question_is_secret());
    }

    #[test]
    fn provider_timeout_can_be_snoozed() {
        let state = QuestionViewState::new("tc".into(), Vec::new(), StashedPrompt::default())
            .with_provider_behavior(HashMap::new(), Some(0));
        assert!(state.auto_resolution_due(Instant::now()));
        let mut state = state;
        state.snooze_auto_resolution();
        assert!(!state.auto_resolution_pending());
    }
}
