...
/// Permission mode's "auto" choice and terminal themes follow their runtime gates.
#[test]
fn enum_choice_gated_off_covers_permission_and_terminal_theme() {
...
fn picker_long_description_wraps_to_multiple_lines() {
    let entries = vec![SettingMeta {
        ...
        description: "Controls how the agent requests permission before it uses tools.",
        keywords: &["test"],
        kind: SettingKind::Enum {
            default: "opt-out",
            choices: &[
                EnumChoice {
                    canonical: "opt-in",
                    display: "Opt in",
                    description: "Allow safe tools to run and ask before higher-risk operations.",
                },
                EnumChoice {
                    canonical: "opt-out",
                    display: "Opt out",
                    description: "Ask before each operation that needs permission from the user.",
                },
            ],
            supports_preview: false,
        },
    ...
    for word in ["Allow", "safe", "tools", "higher-", "risk", "operations"] {
        assert!(
...
fn picker_double_click_selects_radio() {
    let mut s = enter_picker_for("permission_mode");
    ...
    match second {
        SettingsKeyOutcome::Action(Action::SetPermissionMode(_)) => {}
...
fn picker_double_click_deep_link_closes() {
    ...
    match outcome {
        SettingsKeyOutcome::ActionThenClose(Action::SetPermissionMode(_)) => {}
...
fn row_layout_threshold_is_exact() {
    let label = "Permission controls"; // 19 cells
...
// User-feedback follow-up: always reserve a blank line between the agent tip footer
...
fn chevron_column_aligns_across_one_and_two_line_layouts() {
    let theme = Theme::current();
    // `synthetic_enum_chevron_meta` has a 19-cell label and an 8-cell value.
...
fn docs_footer_tip_is_centered() {
    ...
    // Width 45 fits SHORT but not LONG.
    let (row_short, tip_start_short, trailing_short) = render(45);
    assert!(
        row_short.contains("change a setting"),
        "width=45 must render SHORT path (contains `change a setting`): {row_short:?}",
    );
    assert!(
        !row_short.contains("compact mode"),
        "width=45 must not render the long path: {row_short:?}",
    );
    assert!(
...
fn tip_line_has_blank_row_above() {
    ...
    for y in 0..area.height {
        let txt = buf_row_text(&buf, y, area.x, area.width);
        if txt.contains("Tip") && txt.contains("Ask your agent") {
            tip_y = Some(y);
            break;
        }
    }
