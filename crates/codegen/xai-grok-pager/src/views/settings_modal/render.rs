// Modified by the Bot project on 2026-09-13: use provider-neutral settings guidance.
...
pub(super) fn render_docs_footer(buf: &mut Buffer, area: Rect, theme: &Theme) {
    const LONG: &str =
        "Tip · Ask your agent: \"change the theme\" or \"what does compact mode do?\"";
    const SHORT: &str = "Tip · Ask your agent to change a setting";
...
fn render_expanded_description(buf: &mut Buffer, area: Rect, meta: &SettingMeta, theme: &Theme) {
    ...
    let line = Line::from(Span::styled(meta.description, desc_style));
    let wrapped = crate::render::wrapping::word_wrap_line(&line, wrap_w as usize);
