// Modified by the Bot project on 2026-09-12: remove provider-owned setting actions.
...
/// Row visibility for settings that are not available in minimal mode.
/// Pure for unit tests.
pub(super) fn setting_row_visible(meta: &SettingMeta, minimal: bool) -> bool {
    !(minimal && meta.hidden_in_minimal)
}
...
pub(super) fn action_for_enum_commit(key: SettingKey, choice: &'static str) -> Option<Action> {
    match key {
        ...
        "permission_mode" => match choice {
            ...
            "read-only" => Some(Action::SetPermissionMode(
                crate::app::actions::PermissionModeKind::ReadOnly,
            )),
...
/// The gated pairs are `permission_mode`'s "auto" when the auto gate is off and the theme keys' "terminal" while its rollout gate is off.
