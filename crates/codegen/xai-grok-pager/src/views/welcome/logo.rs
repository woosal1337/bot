// Modified by the Bot project on 2026-09-11: product logo assets.
//! The logo is hidden on legacy Windows consoles because their raster fonts do not cover its Unicode block glyphs.
...
const LOGO: &str = include_str!("../../../assets/logo/bot07.txt");
const LOGO_SMALL: &str = include_str!("../../../assets/logo/bot05.txt");
...
/// The block art has no ASCII stand-in; see the module doc.
fn logo_hidden() -> bool {
...
fn render_into(area: Rect, buf: &mut Buffer, theme: &Theme, logo: &str) {
    ...
    // Blend each glyph from the resting gray toward the bright text color by its shine opacity, so a sheen sweeps across the block art
    ...
    let logo_lines: Vec<Line> = lines
        ...
        .map(|(row, line)| {
            ...
            for (col, ch) in line.chars().enumerate() {
                ...
                let diag = (col as f32 + (rows - 1.0 - row as f32)) / (cols as f32 + rows);
            ...
            run.extend(std::iter::repeat_n(' ', cols - line.chars().count()));
            if let Some(prev) = run_color {
...
pub fn render_logo(area: Rect, buf: &mut Buffer, theme: &Theme, window_height: u16) {
    let mut tier = LogoTier::for_height(window_height);
    while tier.width() > area.width {
        let Some(smaller) = tier.step_down() else {
            return;
        };
        tier = smaller;
    }
...
/// Line count of the small logo used in minimal's committed welcome card (0 on a legacy Windows console, where the block art is suppressed).
...
pub fn compact_logo_visual_width() -> u16 {
    if logo_hidden() {
        0
    } else {
        visual_width(LOGO_SMALL)
    }
}

/// Render the small block logo (centered) into `area` for minimal's welcome card.
...
mod tests {
    ...
    // The block art has no legacy-safe stand-in, so every height tier must collapse to no logo when the legacy-console flag is set
    #[test]
    ...
    #[test]
    fn compact_wordmark_snapshot() {
        assert_eq!(visual_width(LOGO), 28);
        assert_eq!(visual_width(LOGO_SMALL), 21);
        let area = Rect::new(0, 0, 21, 5);
        let mut buf = Buffer::empty(area);
        render_into(area, &mut buf, &Theme::current(), LOGO_SMALL);
