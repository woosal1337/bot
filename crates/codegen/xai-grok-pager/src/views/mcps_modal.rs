// Modified by the Bot project on 2026-09-13: provider-neutral MCP sections.
...
/// Display label for a section header, e.g. `"Managed by provider (3)"`.
pub fn section_label(section: &McpSectionId, count: usize) -> String {
    match section {
        McpSectionId::Managed => format!("Managed by provider ({count})"),
...
pub fn section_description_lines(section: &McpSectionId) -> Vec<String> {
    match section {
        McpSectionId::Managed => vec!["Managed by the active provider".into()],
...
mod tests {
    ...
    #[test]
    fn section_description_lines_managed_is_provider_neutral() {
        assert_eq!(
            section_description_lines(&McpSectionId::Managed),
            ["Managed by the active provider"]
        );
    }
