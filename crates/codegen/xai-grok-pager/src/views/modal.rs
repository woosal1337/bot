// Modified by the Bot project on 2026-09-13: hide provider-unsupported extension controls.
...
pub enum ActiveModal {
    ...
    DocViewer {
        ...
        previous_palette: Option<PaletteSnapshot>,
        /// When true, Esc closes the modal directly instead of returning to the document list.
        standalone: bool,
    },
...
pub(crate) fn default_palette_entries(slash: &crate::slash::SlashController) -> Vec<PaletteEntry> {
    ...
    let mut entries = vec![
        ...
        PaletteEntry {
            ...
            shortcut: "/permissions".into(),
            command: PaletteCommand::SlashCommand("/permissions ".into()),
        },
        PaletteEntry {
    ...
    entries.retain(|entry| {
        if let PaletteCommand::OpenExtensionsTab(tab) = &entry.command
            && !tab.is_available()
        {
            return false;
        }
...
fn fit_docs_ask_bot_tip(docs_path: &str, width: usize) -> String {
    use crate::render::line_utils::truncate_str;
    if width == 0 {
        return String::new();
    }
    let long = format!("Tip · Ask Bot about the docs ({docs_path}), e.g. \"how do I set up MCP?\"");
    if long.width() <= width {
        return long;
    }
    let short = format!("Tip · Ask Bot about the docs · {docs_path}");
...
pub fn render_doc_picker_overlay(
    ...
    if let Some(tip_rect) = tip_area {
        ...
        let tip_line = fit_docs_ask_bot_tip(&docs_path, tip_rect.width as usize);
...
mod palette_tests {
    ...
    fn default_palette_includes_dashboard() {
        ...
        assert!(
            entries
                .iter()
                .all(|entry| entry.shortcut.trim() != "/share")
        );
    }
    ...
    #[test]
    fn xai_feedback_is_not_offered_in_the_common_palette() {
        for mode in [
            crate::app::ScreenMode::Minimal,
            crate::app::ScreenMode::Fullscreen,
        ] {
            assert!(
                default_palette_entries(&slash(mode))
                    .iter()
                    .all(|entry| entry.label != "Send Feedback" && entry.shortcut != "/feedback")
            );
        }
    }
    #[test]
...
mod doc_picker_tip_tests {
    use super::{
        ActiveModal, DOCS_USER_GUIDE_REL, fit_docs_ask_bot_tip, howto_list_modal,
    ...
    fn fit_docs_tip_prefers_path_and_never_overflows() {
        ...
        let long = format!("Tip · Ask Bot about the docs ({path}), e.g. \"how do I set up MCP?\"");
        let short = format!("Tip · Ask Bot about the docs · {path}");
        let path_only = format!("Tip · {path}");
        assert_eq!(fit_docs_ask_bot_tip(&path, long.width()), long);
        assert_eq!(fit_docs_ask_bot_tip(&path, short.width()), short);
        assert_eq!(fit_docs_ask_bot_tip(&path, path_only.width()), path_only);
        for w in [5usize, 12, 20, 30] {
            let line = fit_docs_ask_bot_tip(&path, w);
    ...
    fn doc_picker_renders_tip_with_path() {
        ...
        assert!(
            all.contains("Tip") && all.contains("Ask Bot"),
            "missing tip footer:\n{all}"
        );
        assert!(
