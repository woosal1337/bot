...
pub fn mode_flags<'a>(
    ...
    if permission != PermissionLabel::Ask {
        flags.push(PromptFlag {
            text: crate::provider::active_permission_keyword(permission.as_canonical()),
...
impl PromptWidget {
    ...
    pub fn draw(
        ...
        if let Some(mask_character) = style.mask_character {
            self.textarea
                .render_masked(ta_area, buf, &mut self.textarea_state, mask_character);
        } else {
        ...
        let (slash_active, slash_has_inline_ghost) = if style.mask_character.is_some() {
            (false, false)
        } else {
            let snap = self.slash_state.snapshot();
        ...
        let preview_image = if style.mask_character.is_none()
            && style.image_preview
            && style.focused
