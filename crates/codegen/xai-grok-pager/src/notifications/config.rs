...
# session-name, cwd, model, turn-timer, bot
items = [\"action-required\", \"spinner\", \"activity\", \"session-name\", \"bot\"]

# [[ui.notifications.hooks]]
# command = \"terminal-notifier -title 'Bot' -message '$BOT_MESSAGE'\"
# events = [\"turn_complete\", \"approval_required\"]
# only_unfocused = true
# timeout_secs = 10
"
...
mod tests {
    ...
    fn config_round_trips_through_toml() {
        let config = NotificationConfig {
            ...
            title: TitleConfig {
                enabled: false,
                items: vec![TitleItem::Bot, TitleItem::Cwd],
            },
    ...
    #[test]
    fn legacy_grok_title_item_loads_as_bot() {
        let parsed: TitleConfig =
            toml::from_str("enabled = true\nitems = [\"grok\"]\n").expect("legacy title");
        assert_eq!(parsed.items, vec![TitleItem::Bot]);
        let serialized = toml::to_string(&parsed).expect("Bot title");
        assert!(serialized.contains("\"bot\""));
        assert!(!serialized.contains("\"grok\""));
    }
