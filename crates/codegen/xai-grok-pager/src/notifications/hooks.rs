// Modified by the Bot project on 2026-09-13: provide Bot notification variables.
...
fn execute_hook(
    ...
    cmd.arg("-c")
        .arg(command)
        .env("BOT_EVENT", event_str)
        .env("BOT_MESSAGE", message)
...
mod tests {
    ...
    #[test]
    fn sets_legacy_grok_environment_variables() {
    ...
    #[test]
    fn sets_bot_environment_variables() {
        let dir = tempfile::tempdir().unwrap();
        let out = dir.path().join("bot-env.txt");
        let command = format!(
            "printf 'BOT_EVENT=%s\\nBOT_MESSAGE=%s\\nBOT_SESSION_ID=%s\\n' \
             \"$BOT_EVENT\" \"$BOT_MESSAGE\" \"$BOT_SESSION_ID\" > {}",
            out.display()
        );
        ...
        let content = std::fs::read_to_string(&out).unwrap();
        assert!(content.contains("BOT_EVENT=Turn complete"));
        assert!(content.contains("BOT_MESSAGE=hello world"));
        assert!(content.contains("BOT_SESSION_ID=sess-42"));
    }
