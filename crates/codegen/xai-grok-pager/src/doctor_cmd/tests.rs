...
fn mixed_report() -> DiagnosticReport {
    ...
    report.findings = vec![
        ...
        DiagnosticFinding {
            ...
            message: "Use local SSH wrapping".to_owned(),
            remediation: Some(ManualRemediation {
                fix: "bot wrap ssh <host>".to_owned(),
                config_path: None,
            }),
            automatic_remediation: Some(crate::diagnostics::ssh_wrap_automatic_remediation()),
            note: None,
...
fn human_mixed_fixture_is_exact() {
    assert_eq!(
        ...
        concat!(
            ...
            "  Some checks only run in Bot. Start Bot and run /doctor.\n",
            "\n",
...
fn fix_preview_contains_exact_change_and_caveats() {
    ...
    assert!(
        preview.contains(
            "# >>> grok doctor >>>\n# >>> terminal.ssh-wrap >>>\nalias ssh='bot wrap ssh'"
        )
    );
    assert!(preview.contains("To use once without changing config: `bot wrap ssh <host>`"));
...
fn json_contract_is_structural_stable_ordered_and_ansi_free() {
        ...
        serde_json::json!({
            ...
            "findings": [
                ...
                },
                {
                    ...
                    "remediation": {"fix": "bot wrap ssh <host>", "configPath": null},
                    "automaticRemediation": {
                        "fixId": "terminal.ssh-wrap",
                        "command": "bot doctor fix terminal.ssh-wrap"
                    },
                    "note": null
                }
            ],
    ...
    assert!(!text.contains("Bot Doctor"));
}
...
fn new_named_findings_extend_json_without_schema_changes() {
    ...
    report.facts.clipboard.fix = Some("bot wrap <ssh command> or /minimal".to_owned());
    report.findings.push(DiagnosticFinding {
    ...
    assert_eq!(
        ...
        "bot wrap <ssh command> or /minimal"
    );
