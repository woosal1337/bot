// Modified by the Bot project on 2026-09-12: provider-neutral settings catalog.
...
pub struct EnumChoice {
    ...
    /// Display label shown in the chooser (e.g. `"Bot Night"`).
...
impl SettingsRegistry {
    ...
    pub fn for_provider(provider: &crate::provider::ProviderId) -> Self {
        let entries = crate::settings::defs::default_settings_for(provider);
...
pub fn current_value_for(
    ...
    match key {
        ...
        "permission_mode" => Some(SettingValue::Enum(if pager.yolo_mode {
            ...
            "default"
        } else if matches!(ui.permission_mode.as_deref(), Some("read-only")) {
            "read-only"
        } else {
            "ask"
        })),
...
mod tests {
    ...
    #[test]
    fn bot_registry_contains_only_supported_settings() {
        let reg = SettingsRegistry::bot();

        for key in ["theme", "permission_mode", "default_model", "plan_mode"] {
            assert!(
                reg.find(key).is_some(),
                "{key} must remain available in Bot"
            );
        }

        let unsupported_search_results = ["voice", "coding data", "auto-update"]
            .map(|query| {
                let matches = reg
                    .search(query)
                    .into_iter()
        ...
        insta::assert_snapshot!(unsupported_search_results, @r###"
        voice: (none)
        coding data: (none)
        auto-update: (none)
        "###);
    ...
    #[test]
    fn provider_settings_matrix_is_exact() {
        let provider_only = [
        ...
        let grok = SettingsRegistry::for_provider(&crate::provider::ProviderId::Grok);
        let codex = SettingsRegistry::for_provider(&crate::provider::ProviderId::Codex);
        let claude = SettingsRegistry::for_provider(&crate::provider::ProviderId::Claude);

        for key in provider_only {
            assert!(grok.find(key).is_some(), "Grok must expose {key}");
            assert!(codex.find(key).is_none(), "Codex must hide {key}");
            assert!(claude.find(key).is_none(), "Claude must hide {key}");
        }
        ...
        let claude_keys = claude
            ...
            .map(|setting| setting.key)
            .collect::<Vec<_>>();
        assert_eq!(codex_keys, claude_keys);
    }

    #[test]
    fn codex_settings_catalog_snapshot() {
        let keys = SettingsRegistry::for_provider(&crate::provider::ProviderId::Codex)
            .all()
            .iter()
            .map(|setting| setting.key)
            .collect::<Vec<_>>()
    ...
    #[test]
    fn codex_settings_copy_is_provider_neutral() {
        let registry = SettingsRegistry::for_provider(&crate::provider::ProviderId::Codex);
        for setting in registry.all() {
            assert!(!setting.label.contains("Grok"), "{} label", setting.key);
            assert!(
                !setting.description.contains("Grok"),
                "{} description",
                setting.key
            );
            if let SettingKind::Enum { choices, .. } = &setting.kind {
                for choice in *choices {
                    assert!(
                        !choice.display.contains("Grok") && !choice.description.contains("Grok"),
                        "{} choice {}",
                        setting.key,
                        choice.canonical
                    );
