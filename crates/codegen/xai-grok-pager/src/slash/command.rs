...
impl WorkflowChoice {
    ...
    pub fn from_acp(cmd: &acp::AvailableCommand) -> Option<Self> {
        let ownership = command_ownership(cmd)?;
        if ownership.kind() != CommandKind::Workflow
            || cmd.meta.as_ref()?.get("workflowSource")?.as_str()? != ownership.owner()
        {
            return None;
        }
        let description = cmd
...
pub(crate) fn command_ownership(cmd: &acp::AvailableCommand) -> Option<CommandOwnership> {
    serde_json::from_value(cmd.meta.as_ref()?.get("botOwnership")?.clone()).ok()
}
...
pub trait SlashCommand: Send + Sync {
    ...
    /// Refuse before the submit path mutates the composer.
    fn submission_refusal(&self, _args: &str, _is_minimal: bool) -> Option<&'static str> {
        None
    }
