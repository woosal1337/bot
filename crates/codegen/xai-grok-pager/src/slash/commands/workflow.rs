// Modified by the Bot project on 2026-09-13: gate workflow controls by provider capability.
...
impl SlashCommand for WorkflowCommand {
    ...
    fn visible(&self, ctx: &AppCtx) -> bool {
        crate::provider::active_extension_capabilities().workflows && ctx.workflows_available
    }
    ...
    fn run(&self, ctx: &mut CommandExecCtx, args: &str) -> CommandResult {
        if !crate::provider::active_extension_capabilities().workflows {
            return CommandResult::Error(
                "Workflows are not available for this provider.".to_string(),
            );
        }
