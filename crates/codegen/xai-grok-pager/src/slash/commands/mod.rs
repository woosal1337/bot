// Modified by the Bot project on 2026-09-11: provider command registration.
...
pub fn builtin_commands() -> Vec<Arc<dyn SlashCommand>> {
    vec![
        ...
        Arc::new(permissions::PermissionsCommand),
        ...
        Arc::new(account::AccountCommand),
...
mod tests {
    ...
    #[test]
    fn usage_opens_local_session_usage() {
        assert!(matches!(
            run_usage(""),
    ...
    #[test]
    fn disabled_session_share_is_not_a_builtin_command() {
        assert!(
            CommandRegistry::new(builtin_commands())
                .get_for_dispatch("share")
                .is_none()
        );
    }
    #[test]
    fn xai_feedback_is_not_a_builtin_command() {
        assert!(
            CommandRegistry::new(builtin_commands())
                .get_for_dispatch("feedback")
                .is_none()
        );
    }
    #[test]
    fn xai_product_commands_are_not_builtins() {
        let registry = CommandRegistry::new(builtin_commands());
        for command in ["announcements", "release-notes", "changelog"] {
            assert!(registry.get_for_dispatch(command).is_none(), "{command}");
        }
    }
    #[test]
    fn usage_is_provider_neutral() {
        let models = ModelState::default();
        let ctx = crate::slash::command::AppCtx {
            models: &models,
            cwd: std::path::Path::new("."),
