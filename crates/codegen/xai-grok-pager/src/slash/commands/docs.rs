// Modified by the Bot project on 2026-09-13: Removed xAI online documentation routes.
//! `/docs` opens local How-to Guides.
//!
...
impl SlashCommand for DocsCommand {
    slash_meta! {
        ...
        description: "Open local How-to Guides",
        usage: "/docs [title]",
        takes_args: true,
        args_required: false,
    ...
    fn suggest_args(&self, _ctx: &AppCtx, _args_query: &str) -> Option<Vec<ArgItem>> {
        let mut items = vec![ArgItem {
            ...
            description: "Browse local How-to Guides".into(),
        }];
    ...
    fn run(&self, _ctx: &mut CommandExecCtx, args: &str) -> CommandResult {
        ...
        match find_doc(trimmed) {
            Some(doc) => CommandResult::Action(Action::ShowDocument {
                title: doc.title.into(),
                content: doc.content.into(),
            }),
            None => CommandResult::Error(format!(
                "Unknown docs target {trimmed:?}. Try /docs or a guide title, for example /docs Getting Started."
            )),
        }
    }
}
...
mod tests {
    ...
    fn title_opens_guide() {
        ...
        match DocsCommand.run(&mut ctx, "Getting Started") {
            CommandResult::Action(Action::ShowDocument { title, content }) => {
    ...
    #[test]
    fn suggest_args_include_local_guides_only() {
        let models = ModelState::default();
        ...
        assert!(items.iter().all(|i| i.insert_text != "web"));
