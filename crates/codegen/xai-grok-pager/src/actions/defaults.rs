...
pub(super) fn default_actions(
    ...
    let (
        cycle_mode_description,
        cycle_mode_help,
        yolo_label,
        yolo_description,
        yolo_help,
        dashboard_yolo_label,
        dashboard_yolo_description,
        dashboard_yolo_help,
    ) = if crate::provider::active_provider() == crate::provider::ProviderId::Codex {
        (
            "Cycle mode (Ask for approval / Plan / Approve for me / Full Access)",
            "Steps the session mode: Ask for approval -> Plan -> Approve for me -> Full Access -> Ask for approval.\nPlan keeps the agent planning first and writes no files. Full Access turns off approval prompts and the sandbox.\nCtrl+O toggles Full Access directly.",
            "full access",
            "Toggle Full Access",
            "Turns Full Access on or off for this Codex session.\nWhile on, approval prompts and the sandbox are off, so tools can access the whole machine.\nUse with care.",
            "full-access",
            "Toggle Full Access",
            "Toggles Full Access for the selected Codex agent without attaching to it.\nWhile on, approval prompts and the sandbox are off, so tools can access the whole machine.\nThe per-session equivalent is Ctrl+O inside a session.",
        )
    } else {
        (
            "Cycle mode (Ask / Plan / Auto / Always-approve)",
            "Steps the session mode: Ask -> Plan -> Auto -> Always-approve -> Ask.\nPlan keeps the agent planning first and writes no files. Always-approve skips approval prompts.\nCtrl+O toggles Always-approve directly.",
            "yolo",
            "Toggle Always-approve",
            "Turns Always-approve on or off for this session.\nWhile on, the agent runs tool calls without approval prompts. Deny rules, hooks, and the sandbox still apply.\nUse with care.",
            "always-approve",
            "Toggle Always-approve",
            "Toggles Always-approve for the selected agent without attaching to it.\nWhile on, the agent runs tool calls without approval prompts. Deny rules, hooks, and the sandbox still apply.\nThe per-session equivalent is Ctrl+O inside a session.",
        )
    };
    ...
    let mut actions = vec![
        ...
        ActionDef {
            ...
            label: "mode",
            description: cycle_mode_description,
            ...
            requires_confirmation: false,
            long_help: Some(cycle_mode_help),
        },
    ...
    actions.extend([
        ...
        ActionDef {
            ...
            label: dashboard_yolo_label,
            description: dashboard_yolo_description,
            default_key: key!('o', CONTROL),
            alt_keys: vec![],
            category: Category::Dashboard,
            context: When::DashboardFocused,
            ...
            requires_confirmation: false,
            long_help: Some(dashboard_yolo_help),
        },
