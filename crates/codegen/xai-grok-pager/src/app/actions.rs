...
pub enum Action {
    ...
    SwitchProvider(crate::provider::ProviderId),
    ...
    OpenSettings,
    /// Open settings on a registry key.
    OpenSettingsFocus {
        key: &'static str,
    },
    ...
    Login,
    LoginBrowser,
    /// User selected device-code login.
    LoginDevice,
    ...
    ShowSessionInfo,
    /// Show the current provider account and rate limits.
    ShowAccount,
    /// Show a local document in a modal.
    ShowDocument {
        title: String,
        content: String,
    },
...
impl From<PermissionLabel> for PermissionModeKind {
    fn from(label: PermissionLabel) -> Self {
        match label {
            ...
            PermissionLabel::ReadOnly => Self::ReadOnly,
...
impl std::fmt::Display for PermissionLabel {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str(crate::provider::active_permission_keyword(
            self.as_canonical(),
        ))
    }
}
...
mod permission_mode_kind_tests {
    ...
    fn permission_mode_choices_include_auto_in_catalog() {
        for c in ["default", "ask", "auto", "read-only", "always-approve"] {
            assert!(
...
pub enum Effect {
    ...
    FetchBundleStatus,
    /// Fetch the current provider account via `x.ai/account/read`.
    FetchAccountStatus,
...
pub enum TaskResult {
    ...
    SessionModeSet {
        ...
        unsent_prompt: Option<(AgentId, String)>,
    },
    ...
    AccountStatusLoaded {
        result: Result<crate::app::account::ProviderAccountStatus, String>,
    },
    ...
    AvailableCommandsRefreshed {
        agent_id: AgentId,
        result: Result<Vec<acp::AvailableCommand>, String>,
    },
    /// Provider logout finished.
    LogoutComplete {
        result: Result<(), String>,
    },
