// Modified by the Bot project on 2026-09-12: command identity and the launcher provider default.
...
pub enum Command {
    /// Run Bot without the interactive UI
    Agent(Box<AgentArgs>),
    /// Show the configuration Bot discovers for this directory
    Inspect {
        /// Emit machine-readable JSON output.
        #[arg(long)]
        json: bool,
    },
    /// Check terminal, clipboard, color, and input support without starting Bot
...
Examples:
  bot wrap docker exec -it my-container bash
  bot wrap kubectl exec -it my-pod -- bash
...
/// Targets a running leader process by PID (used by `bot leader` / `bot workspace`).
#[derive(Debug, clap::Args, Clone, Default)]
pub struct LeaderTargetArgs {
    /// Leader process ID from `bot leader list`.
    #[arg(long)]
    pub pid: Option<u32>,
}
#[derive(Debug, clap::Args, Clone)]
...
#[command(
    ...
    about = "Bot terminal agent interface",
...
pub struct PagerArgs {
    ...
    /// Name it `~/.grok/leader-*.sock` to keep `bot leader list/kill` able to find it; any other location works but won't be auto-discovered
    #[arg(
        long = "leader-socket",
        value_name = "PATH",
        global = true,
    ...
    #[arg(
        ...
        value_parser = clap::builder::PossibleValuesParser::new(
            crate::provider::PERMISSION_MODE_CLI_VALUES
        )
    )]
    ...
    pub no_leader: bool,
    /// Initial prompt for the interactive session, e.g. `bot "fix the bug"` or `bot --worktree=feat "create this feature"`.
    #[arg(
        value_name = "PROMPT",
        conflicts_with_all = &["single",
        "prompt_json",
...
impl PagerArgs {
    ...
    /// This is the `bot "<prompt>"` launch form; the headless `-p`/`--single` path is handled separately.
...
mod tests {
    use super::*;
    #[test]
    fn disabled_session_share_is_not_a_cli_command() {
        use clap::CommandFactory;

        assert!(
            PagerArgs::command()
                .get_subcommands()
                .all(|command| command.get_name() != "share")
        );
    }

    #[test]
    fn provider_accepts_the_launcher_default_and_explicit_choices() {
        use clap::CommandFactory;
        ...
        assert_eq!(
            provider.get_env(),
            Some(std::ffi::OsStr::new("BOT_PROVIDER"))
        );
        assert_eq!(
            provider
                .get_default_values()
        ...
        for provider in ["grok", "codex"] {
            let args = PagerArgs::try_parse_from(["bot", "--provider", provider]).unwrap();
    ...
    #[test]
    fn permission_mode_accepts_provider_native_keywords() {
        for value in [
            "ask",
            "auto",
            "always-approve",
            ...
            let args = PagerArgs::try_parse_from(["bot", "--permission-mode", value])
                .expect("permission mode parses");
            assert_eq!(args.permission_mode_flag.as_deref(), Some(value));
        }
    }
    #[test]
