...
/// Rate-limit errors render safe provider detail or an auth-aware fallback (see [`format_rate_limited_user_message`]).
...
pub(crate) async fn persist_permission_mode_and_notify(
    ...
    if notify_first {
        notify(tx.clone(), canonical, session_id.as_ref()).await;
    }
