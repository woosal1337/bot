    ...
    match effect {
        ...
        Effect::Logout => {
            ...
            tasks.spawn(async move {
                let result = send_logout(&tx).await;
                TaskResult::LogoutComplete { result }
            });
        }
            ...
            let abort_handle = tasks
                .spawn(async move {
                    match send_logout(&tx).await {
                        Ok(()) => {
                            send_authenticate(&tx, request_seq, method_id, use_oauth, false).await
                        }
                        Err(error) => TaskResult::AuthFailed { request_seq, error },
                    }
                });
        ...
        Effect::SetSessionMode { session_id, mode_id } => {
            ...
            tasks
                .spawn(async move {
                    let req = acp::SetSessionModeRequest::new(session_id.clone(), mode_id.clone());
                    let result: Result<acp::SetSessionModeResponse, _> = acp_send(req, &tx).await;
            ...
            tasks
                .spawn(async move {
                    ...
                    if let Err(e) = acp_send(mode_req, &tx).await {
                        return TaskResult::SessionModeSet {
                            session_id,
                            mode_id,
                            result: Err(e.to_string()),
                            unsent_prompt: Some((agent_id, prompt_id)),
                        };
                    }
                    ulog::info(
                        "prompt submitted",
        ...
        Effect::PersistModelEffortDefault {
            provider,
            model,
            effort,
        } => {
            tasks.spawn(async move {
                match xai_grok_shell::util::config::set_model_effort_default(
                    provider.clone(),
                    model.clone(),
                    effort.clone(),
                )
                .await
                {
                    Ok(()) => TaskResult::ModelEffortDefaultPersisted {
                        provider,
                        model,
                        effort,
                    },
                    Err(error) => TaskResult::ModelEffortDefaultPersistFailed {
                        error: error.to_string(),
                    },
                }
            });
        ...
        Effect::ToggleSkill { agent_id, session_id, skill_name, enabled } => {
            let tx = acp_tx.clone();
            tasks
                .spawn(async move {
                    let params = serde_json::json!({
        ...
        Effect::FetchAccountStatus => {
            let tx = acp_tx.clone();
            tasks.spawn(async move {
                let request = acp::ExtRequest::new(
                    "x.ai/account/read",
                    serde_json::value::to_raw_value(&serde_json::json!({}))
                        .expect("serialize account/read params")
                        .into(),
                );
                    ...
                    Ok(response) => {
                        let parsed = serde_json::from_str::<serde_json::Value>(response.0.get())
                            .and_then(|value| {
                                ...
                                serde_json::from_value::<super::account::ProviderAccountStatus>(
                                    inner.clone(),
                                )
                            });
                        match parsed {
                            Ok(status) => Ok(status),
                            Err(error) => {
                                tracing::warn!(%error, "provider returned invalid account status");
                                Err("Bot could not read the provider account response.".to_owned())
                            }
                        }
                    }
...
fn session_info_fields(
    ...
    push("Shell version", xai_grok_version::display_version(), false);
