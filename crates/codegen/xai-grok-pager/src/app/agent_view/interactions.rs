// Modified by the Bot project on 2026-09-13: Removed xAI upsell question state.
...
impl AgentView {
            ...
            QuestionFocus::Navigation => {
                ...
                match key.code {
                    ...
                    KeyCode::Char('z') if key.modifiers.is_empty() => {
                        if qv.allows_freeform(qv.active_tab) {
    ...
    pub(super) fn handle_question_mouse(&mut self, mouse: &MouseEvent) -> InputOutcome {
        ...
        match mouse.kind {
            ...
            MouseEventKind::Down(MouseButton::Left) => {
                ...
                let on_sticky_freeform = qv.allows_freeform(qv.active_tab)
                    && qv.focus != crate::views::question_view::QuestionFocus::InputMode
                ...
                if let Some((idx, option_count)) = hit_idx.zip(option_count) {
                    if !qv.allows_freeform(qv.active_tab) && idx >= option_count {
                        return InputOutcome::Changed;
                    }
                    let active_tab = qv.active_tab;
                    qv.set_cursor(idx);
    ...
    fn question_item_at(&self, col: u16, row: u16) -> Option<usize> {
        ...
        if let Some((scroll_top, scroll_bottom)) = self.question_scroll_region {
            ...
            if !is_input_mode && qv.allows_freeform(qv.active_tab) && row == scroll_bottom {
        ...
        .filter(|&idx| qv.allows_freeform(qv.active_tab) || idx < question.options.len())
    }
    ...
    pub(crate) fn dashboard_answer_question(
        ...
        match option_idx {
            ...
            None => {
                if !qv.allows_freeform(active) || freeform.trim().is_empty() {
                    self.question_view = Some(qv);
                    return PeekAnswerOutcome::NoOp;
                }
    ...
    pub(crate) fn resolve_question_timeout(&mut self) -> InputOutcome {
        self.submit_question_answers(false)
    }
}
#[cfg(test)]
...
mod question_no_freeform_tests {
    //! Freeform ("Other") gating for `no_freeform` question modals.
    //! Regression tests for the bug where clicking under the last option selected the hidden freeform row.
    ...
    fn fixed_question() -> Question {
        ...
        Question {
            question: "Continue with this action?".into(),
            options: vec![
                opt("Continue", "Run the action"),
                opt("Cancel", "Keep the current state"),
            ],
