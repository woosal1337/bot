// Modified by the Bot project on 2026-09-13: Removed retired changelog tests.
use super::*;
...
#[test]
fn apply_auth_meta_records_team_users() {
    let mut app = test_app();
...
#[test]
fn welcome_ctrl_u_resumes_foreign_session() {
    let mut app = test_app();
...
fn welcome_doc_viewer_is_scroll_blocking_and_wheel_scrolls_content() {
    ...
    assert!(
        ...
        "the welcome document overlay must block background scroll",
    );
    ...
    assert!(
        app.last_scroll_pos.is_none(),
        "the wheel must not reach the background while a document is open",
    );
...
fn v1_overlay_ctrl_x_background_work_retains_close_confirmation() {
    ...
    assert!(matches!(outcome, InputOutcome::Changed));
    assert!(app.pending_action.is_some());
    assert_eq!(app.agents[&id].session.scheduled_tasks.len(), 1);
}
#[test]
