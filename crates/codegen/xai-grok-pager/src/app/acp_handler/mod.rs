// Modified by the Bot project on 2026-09-13: Removed provider-specific announcement updates.
...
fn workflow_commands(
    ...
    commands
        .iter()
        .filter_map(|command| {
            let ownership = crate::slash::command::command_ownership(command)?;
            if ownership.kind() != bot_provider::CommandKind::Workflow {
                return None;
            }
...
pub(super) fn is_builtin_workflow_handle(
    ...
    let is_builtin = |command: &acp::AvailableCommand| {
        crate::slash::command::command_ownership(command).is_some_and(|ownership| {
            ownership.kind() == bot_provider::CommandKind::Workflow
                && ownership.owner() == "builtin"
