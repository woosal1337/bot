...
pub(super) fn dispatch_login(app: &mut AppView) -> Vec<Effect> {
    ...
    start_login(app, method_id, app.auth_start_mode)
}

pub(super) fn dispatch_device_login(app: &mut AppView) -> Vec<Effect> {
    dispatch_login_mode(app, "device", AuthMode::Device)
}

pub(super) fn dispatch_browser_login(app: &mut AppView) -> Vec<Effect> {
    dispatch_login_mode(app, "browser", AuthMode::Command)
}

fn dispatch_login_mode(app: &mut AppView, auth_mode: &str, mode: AuthMode) -> Vec<Effect> {
    let method_id = app.auth_methods.iter().find_map(|method| {
        (method
            .meta()
            .as_ref()
            .and_then(|meta| meta.get("auth_mode"))
    ...
    let Some(method_id) = method_id else {
        app.auth_state = AuthState::Pending {
            error: Some(format!(
                "{} login is not available for this provider",
                if auth_mode == "device" {
                    "Device-code"
                } else {
                    "Browser"
