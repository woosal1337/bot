...
use super::auth::{
    dispatch_browser_login, dispatch_cancel_login, dispatch_device_login, dispatch_login,
...
use super::status::{
    dispatch_copy_session_id, dispatch_open_gboom, dispatch_open_tutorial, dispatch_show_account,
    dispatch_show_context_info, dispatch_show_document, dispatch_show_queue,
    ...
    let effects = match action {
        ...
        Action::RelaunchInScreenMode { minimal } => {
            ...
            if let Some(session_id) = app.active_session_id().map(str::to_owned) {
                app.relaunch = Some(crate::app::app_view::ProcessRelaunch::ScreenMode(
                    crate::app::app_view::ScreenModeRelaunch {
                        minimal,
                        session_id,
                    },
                ));
        ...
        Action::SwitchProvider(provider) => {
            app.relaunch = Some(crate::app::app_view::ProcessRelaunch::Provider(provider));
            let mut effects = unregister_all_active_sessions(app);
            effects.push(Effect::Quit);
            effects
        }
        ...
        Action::ReloadSkills => {
            ...
            let workflows_available = agent.extensions_modal.as_ref().is_some_and(|modal| {
                modal
                    .tabs
                    .contains(&crate::views::extensions_modal::ExtensionsTab::Workflows)
            });
            ...
            let mut effects = vec![Effect::FetchSkillsList {
                agent_id: id,
                session_id: session_id.clone(),
            }];
            if workflows_available {
                effects.push(Effect::FetchWorkflowsList {
                    agent_id: id,
                    session_id,
                });
            }
        ...
        Action::ShowAccount => dispatch_show_account(app),
        Action::ShowDocument { title, content } => dispatch_show_document(app, title, content),
        ...
        Action::LoginBrowser => dispatch_browser_login(app),
        Action::LoginDevice => dispatch_device_login(app),
    ...
    match result {
        Err(e) => {
            if let Some(ref mut modal) = agent.extensions_modal {
                ...
                if !modal.fail_load(source.tab(), e.clone()) {
                    modal.modal_message =
                        Some(crate::views::extensions_modal::ModalMessage::Error(e));
                }
            }
            vec![]
        }
        Ok(outcome) => match outcome.status {
            OutcomeStatus::Success => {
                ...
                if let Some(ref mut modal) = agent.extensions_modal {
                    if !outcome.message.trim().is_empty() && modal.result_notice.is_none() {
                        let entry_index = match (source, modal.last_plugins_action.as_ref()) {
                            (
                                ExtensionActionSource::Plugins,
                ...
                if let Some(session_id) = agent.session.session_id.clone() {
                    if outcome.requires_reload {
                        ...
                        if matches!(
                            source,
                            ExtensionActionSource::Plugins | ExtensionActionSource::Marketplace
                        ) {
                        ...
                        if let Some(modal) = agent.extensions_modal.as_mut() {
                            match source {
                                ExtensionActionSource::Hooks => {
                                    if modal.capabilities.hooks {
                                ...
                                ExtensionActionSource::Plugins
                                | ExtensionActionSource::Marketplace => {
                                    effects.extend(
                                    crate::app::dispatch::transcript::extensions_modal_action_refreshes(
                                        modal,
                                        agent_id,
                                        session_id,
                                    ),
            ...
            OutcomeStatus::ConfirmationRequired => {
                if let Some(ref mut modal) = agent.extensions_modal {
                    let confirmed_action = (source == ExtensionActionSource::Plugins)
            ...
            | OutcomeStatus::Unsupported => {
                if let Some(ref mut modal) = agent.extensions_modal {
                    modal.pending_action = None;
                    modal.pending_entry_index = None;
                    if !modal.fail_load(source.tab(), outcome.message.clone()) {
                        modal.modal_message = Some(
                            crate::views::extensions_modal::ModalMessage::Error(outcome.message),
                        );
                    }
...
pub(super) enum ExtensionActionSource {
    Hooks,
    Plugins,
    Marketplace,
}

impl ExtensionActionSource {
    fn tab(self) -> crate::views::extensions_modal::ExtensionsTab {
        use crate::views::extensions_modal::ExtensionsTab;
        match self {
            Self::Hooks => ExtensionsTab::Hooks,
            Self::Plugins => ExtensionsTab::Plugins,
            Self::Marketplace => ExtensionsTab::Marketplace,
        }
    }
}
