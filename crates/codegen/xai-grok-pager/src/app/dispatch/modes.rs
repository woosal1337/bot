// Modified by the Bot project on 2026-09-12: wait for mode confirmation before showing success.
...
pub(super) fn set_plan_mode(
    ...
    if prev == new {
        let message = if agent.plan_mode_pending.is_some() {
            "Changing plan mode...".to_owned()
        } else {
            plan_mode_toast(kind)
        };
    ...
    app.show_toast("Changing plan mode...");
...
pub(super) fn sync_active_auto_flag(app: &mut AppView) {
        ...
        && let Some(agent) = app.agents.get_mut(&id)
    {
        ...
        agent.session.read_only_mode =
            app.current_ui.permission_mode.as_deref() == Some("read-only");
    }
...
pub(super) fn set_yolo_mode_inner(app: &mut AppView, new: bool) {
    ...
    agent.session.read_only_mode = false;
...
pub(super) fn permission_mode_toast(kind: crate::app::actions::PermissionModeKind) -> String {
    ...
    match kind {
        ...
        PermissionModeKind::Auto => format!(
            "\u{2713} Permission mode: {}",
            crate::provider::active_permission_display("auto")
        ),
        PermissionModeKind::ReadOnly => format!(
            "\u{2713} Permission mode: {}",
            crate::provider::active_permission_display("read-only")
        ),
        PermissionModeKind::Ask => format!(
            "\u{2713} Permission mode: {}",
            crate::provider::active_permission_display("ask")
        ),
...
pub(super) fn yolo_on_under_plan_toast() -> String {
    format!(
        "\u{26A0} {} ON: plan mode still blocks file edits until you exit plan mode",
        crate::provider::active_permission_display("always-approve")
    )
}
...
fn yolo_toast(new: bool) -> String {
    if new {
        if crate::provider::active_provider() == crate::provider::ProviderId::Codex {
            "\u{26A0} Full Access: approval prompts and the sandbox are off".to_string()
        } else {
