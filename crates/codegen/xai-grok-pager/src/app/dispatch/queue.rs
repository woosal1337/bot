                ...
                registry
                    ...
                    .is_some_and(|command| {
                        command
                            ...
                            .is_some()
                            || command
                                .submission_refusal(invocation.args, screen_mode.is_minimal())
                                .is_some()
                    })
            });
        if command_refused {
...
mod tests {
    ...
    fn drain_blocked_when_editing_front_prompt() {
        ...
        // Turn ends: must not drain "second" while the user edits it.
    ...
    fn drain_not_blocked_when_editing_non_front_prompt() {
        ...
        // Turn ends: drains "second" because it is the unedited front item.
        let effects = dispatch(end_turn(), &mut app);
    ...
    fn drain_after_editing_sends_correct_prompt() {
        ...
        // End turn for p2: must not drain p3 while the user edits it.
        let effects = dispatch(end_turn(), &mut app);
        assert!(effects.is_empty());
