// Modified by the Bot project on 2026-09-12: remove provider-owned settings state.
...
pub(super) fn dispatch_send_prompt_submission(
    ...
    if !literal && text.trim().starts_with('/') {
        ...
        if let Some(invocation) = crate::slash::parse_invocation(slash_input.trim())
            ...
            && let Some(refusal) =
                command.submission_refusal(invocation.args, app.screen_mode.is_minimal())
        {
...
pub(super) fn handle_prompt_response(
    ...
    if let Some(agent) = app.agents.get_mut(&agent_id) {
        ...
        // The cases: rate limit, model incompatibility, 401 re-auth, context overflow, and disk-full
        ...
        // This must run before the reconnect early return below, which skips the fetch gate entirely
