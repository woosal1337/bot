// Modified by the Bot project on 2026-09-13: provider-neutral extension tests.
//! Tests for login, logout, account switching, and auth-code dispatchers.

use super::*;

#[test]
fn failed_logout_keeps_the_active_session() {
    let mut app = test_app_with_agent();

    let effects = dispatch(
        Action::TaskComplete(TaskResult::LogoutComplete {
            result: Err("Provider rejected the request.".to_owned()),
        }),
        &mut app,
    );
    ...
    assert!(read_toast(&app).contains("Could not log out"));
}
...
#[test]
fn device_login_selects_the_provider_device_method() {
    let mut app = test_app();
    let browser = acp::AuthMethod::Agent(
        acp::AuthMethodAgent::new("codex.chatgpt", "Sign in with ChatGPT").meta(Some(
            serde_json::json!({
    ...
    let device = acp::AuthMethod::Agent(
        acp::AuthMethodAgent::new("codex.chatgpt.device", "Sign in with a device code").meta(Some(
            serde_json::json!({
                "external_provider": true,
                "auth_mode": "device",
            })
    ...
    app.auth_methods = vec![browser, device];

    let effects = dispatch(Action::LoginDevice, &mut app);
    ...
    assert!(effects.iter().any(|effect| matches!(
        ...
        Effect::Authenticate { method_id, .. }
            if method_id.0.as_ref() == "codex.chatgpt.device"
    )));
}

#[test]
fn browser_login_selects_the_provider_browser_method() {
    let mut app = test_app();
    let device = acp::AuthMethod::Agent(
        acp::AuthMethodAgent::new("codex.chatgpt.device", "Sign in with a device code").meta(Some(
            serde_json::json!({
    ...
    app.auth_methods = vec![device, browser];

    let effects = dispatch(Action::LoginBrowser, &mut app);
    ...
    assert!(effects.iter().any(|effect| matches!(
        effect,
        Effect::Authenticate { method_id, .. }
            if method_id.0.as_ref() == "codex.chatgpt"
    )));
}
