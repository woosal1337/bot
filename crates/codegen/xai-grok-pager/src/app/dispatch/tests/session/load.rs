// Modified by the Bot project on 2026-09-11: verify provider-owned session loading.
...
#[test]
fn pick_provider_owned_row_dispatches_direct_load() {
    let mut app = test_app_with_agent();
    let mut entry = make_picker_entry("provider-thread", "/provider/workspace");
    entry.source = "provider".into();
    open_session_picker_with(&mut app, vec![entry]);
    let effects = dispatch(Action::PickSession(0), &mut app);
    assert!(
        matches!(
            &effects[..],
            [Effect::LoadSession {
                ...
                chat_kind: false,
                ..
            }] if session_id == "provider-thread" && cwd == std::path::Path::new("/provider/workspace")
        ),
        "expected a direct provider LoadSession, got {effects:?}"
    );
}
