// Modified by the Bot project on 2026-09-12: mode confirmation and failure regression tests.
    ...
    dispatch_task_result(
        TaskResult::DoctorFixPlanned {
            ...
            result: Ok(crate::app::actions::DoctorPlanningOutcome::RunLocally(
                "bot doctor fix ssh-wrap".to_owned(),
            )),
        },
        &mut app,
    );
    assert!(
        last_system_text(&app, id).contains("On your local computer, run: bot doctor fix ssh-wrap")
    );
...
#[test]
fn plan_mode_waits_for_confirmation_and_clears_failures() {
    for succeeds in [false, true] {
        let mut app = test_app_with_agent();
        let session_id = app.agents[&AgentId(0)].session.session_id.clone().unwrap();
        dispatch(
            Action::SetPlanMode(crate::app::actions::PlanModeKind::On),
            &mut app,
        );
        assert!(!app.agents[&AgentId(0)].plan_mode_active);
        assert!(read_toast(&app).contains("Changing plan mode"));
        dispatch(
        ...
        assert_eq!(app.agents[&AgentId(0)].plan_mode_active, succeeds);
        assert_eq!(app.agents[&AgentId(0)].plan_mode_pending, None);
        assert!(read_toast(&app).contains(if succeeds {
            "Plan mode: on"
        } else {
            "test rejection"
        }));
...
#[test]
fn mode_failure_finishes_the_unsent_prompt() {
    let mut app = test_app_with_agent();
    let session_id = app.agents[&AgentId(0)].session.session_id.clone().unwrap();
    let effects = dispatch(
    ...
    let (agent_id, prompt_id) = effects
        .iter()
    ...
    dispatch(
        Action::TaskComplete(TaskResult::SessionModeSet {
            session_id,
            mode_id: acp::SessionModeId::new("plan"),
            result: Err("Mode is not available".into()),
...
#[test]
fn available_commands_refreshed_empty_clears_the_catalog() {
    let mut app = test_app_with_agent();
    let id = AgentId(0);
    app.agents.get_mut(&id).unwrap().session.available_commands =
        vec![acp::AvailableCommand::new("old-skill", "Old skill")];
    let gen_before = app.agents[&id].session.available_commands_generation;
...
#[test]
fn available_commands_refresh_error_keeps_the_catalog() {
    let mut app = test_app_with_agent();
    let id = AgentId(0);
    app.agents.get_mut(&id).unwrap().session.available_commands =
        vec![acp::AvailableCommand::new("old-skill", "Old skill")];
