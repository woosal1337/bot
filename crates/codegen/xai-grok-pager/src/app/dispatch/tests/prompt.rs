// Modified by the Bot project on 2026-09-13: Removed announcement-only prompt tests.
...
fn multiple_queued_prompts_drain_one_per_turn() {
    ...
    // Turn end: no queued work remains.
    let effects = dispatch(end_turn(), &mut app);
    assert!(effects.is_empty());
    assert!(app.agents[&id].session.state.is_idle());
}
...
/// Turn end with prompt suggestions enabled fires the `x.ai/suggestPrompt` fetch.
