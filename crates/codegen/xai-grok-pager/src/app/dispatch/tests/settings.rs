...
/// Full path: after a focused open, Esc dismisses the settings modal.
#[test]
...
/// Full path: after a focused open, Enter commits and dismisses.
#[test]
...
fn set_theme_toast_format_uses_display_name() {
    with_theme_test_env(|| {
        ...
        assert!(
            toast.contains("Bot Day"),
            "toast must use display name `Bot Day`, not canonical `grokday`, got: {toast:?}",
        );
...
fn set_auto_light_theme_toast_format_uses_display_name() {
    with_theme_test_env(|| {
        ...
        assert!(toast.contains("Bot Night"));
    });
}
