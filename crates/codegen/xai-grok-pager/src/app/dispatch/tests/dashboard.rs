...
#[serial_test::serial(GROK_AGENT_DASHBOARD)]
#[test]
fn dashboard_slash_usage_is_available_for_external_auth() {
    let mut app = three_agent_app();
    ...
    assert!(
        app.dashboard.as_ref().unwrap().error_toast.is_none(),
        "provider-neutral usage must not show an error"
    );
...
fn dashboard_slash_usage_opens_dashboard_modal() {
    ...
    assert_eq!(modal.active_tab, UsageInfoTab::SessionUsage);
