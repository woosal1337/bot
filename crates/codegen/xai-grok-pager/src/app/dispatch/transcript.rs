// Modified by the Bot project on 2026-09-13: gate extension tabs by provider capability.
...
pub(super) fn extensions_modal_tab_fetches(
    ...
    extensions_modal_fetches(modal, agent_id, session_id, true)
}

pub(super) fn extensions_modal_action_refreshes(
    modal: &mut crate::views::extensions_modal::ExtensionsModalState,
    agent_id: AgentId,
    session_id: acp::SessionId,
) -> Vec<Effect> {
    extensions_modal_fetches(modal, agent_id, session_id, false)
}

fn extensions_modal_fetches(
    modal: &mut crate::views::extensions_modal::ExtensionsModalState,
    agent_id: AgentId,
    session_id: acp::SessionId,
    mcp_cache: bool,
    ...
    if modal.tabs.contains(&ExtensionsTab::Hooks) {
        effects.push(Effect::FetchHooksList {
            agent_id,
            session_id: session_id.clone(),
        });
    }
    if modal.tabs.contains(&ExtensionsTab::Plugins) {
        effects.push(Effect::FetchPluginsList {
            agent_id,
            session_id: session_id.clone(),
        });
    }
    if modal.tabs.contains(&ExtensionsTab::McpServers) {
    ...
    if modal.tabs.contains(&ExtensionsTab::Skills) {
        effects.push(Effect::FetchSkillsList {
            agent_id,
            session_id: session_id.clone(),
        });
    }
    if modal.tabs.contains(&ExtensionsTab::Workflows) {
...
pub(super) fn push_marketplace_fetch(
    ...
    if !modal
        .tabs
        .contains(&crate::views::extensions_modal::ExtensionsTab::Marketplace)
    {
        return;
    }
...
pub(super) fn dispatch_open_extensions_modal(
    ...
    if !tab.is_available() {
        app.show_toast(&format!(
            ...
            tab.label(),
            crate::provider::active_provider().label()
        ));
        return vec![];
    }
    ...
    agent.extensions_modal = Some(ExtensionsModalState::new(tab));
...
pub(super) fn handle_skills_toggle_done(
    ...
    let refresh = result
        .is_ok()
        .then(|| agent.session.session_id.clone())
        .flatten();
    if let Some(ref mut modal) = agent.extensions_modal {
        modal.pending_action = None;
        modal.pending_entry_index = None;
    ...
    refresh
        .map(|session_id| Effect::RefreshAvailableCommands {
            agent_id,
            session_id,
        })
        .into_iter()
