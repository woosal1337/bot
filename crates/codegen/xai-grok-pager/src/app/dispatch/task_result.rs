// Modified by the Bot project on 2026-09-13: Removed the xAI changelog service.
...
use super::router::{ExtensionActionSource, dispatch, dispatch_action_result};
    ...
    for notice in transition.notices {
        match notice {
            ...
            WorkspaceNotice::ReadOnly => {
                app.show_toast("Dashboard workspace is read-only in this Bot version");
            }
...
pub(super) fn dispatch_task_result(result: TaskResult, app: &mut AppView) -> Vec<Effect> {
    ...
    match result {
        ...
        TaskResult::SessionModeSet {
            session_id,
            mode_id,
            result,
            unsent_prompt,
        } => {
            let requested_plan = mode_id.0.as_ref() == "plan";
            let error = result.err();
            if let Some(agent) = find_agent_by_session_id(&mut app.agents, session_id.0.as_ref())
                && agent
                    .plan_mode_pending
                    .is_none_or(|pending| pending == requested_plan)
            {
                agent.plan_mode_pending = None;
                if let Some(error) = error.as_ref() {
                    agent.show_toast(&format!(
                        "Cannot change mode: {error}. Retry the mode change."
                    ));
                } else {
                    agent.plan_mode_active = requested_plan;
                    agent.show_toast(&super::settings::ui::save_success_toast(
                        "Plan mode",
                        requested_plan,
                    ));
                }
            ...
            if let (Some(error), Some((agent_id, prompt_id))) = (error, unsent_prompt) {
                return handle_prompt_response(
                    app,
                    agent_id,
                    Err(format!(
                        "Mode change failed. The prompt was not sent: {error}"
                    )),
                    None,
                    Some(prompt_id),
                );
        ...
        TaskResult::AccountStatusLoaded { result } => {
            let content = match result {
                Ok(status) => status.render_markdown(),
                Err(error) => format!("# Account\n\nCould not load account status.\n\n{error}"),
            };
            super::status::dispatch_show_document(app, "Account".to_owned(), content)
        }
        ...
        TaskResult::HooksActionResult { agent_id, result } => {
            dispatch_action_result(app, agent_id, ExtensionActionSource::Hooks, result)
        }
        TaskResult::PluginsActionResult { agent_id, result } => {
            dispatch_action_result(app, agent_id, ExtensionActionSource::Plugins, result)
        }
        TaskResult::MarketplaceActionResult { agent_id, result } => {
            dispatch_action_result(app, agent_id, ExtensionActionSource::Marketplace, result)
        }
        ...
        TaskResult::AvailableCommandsRefreshed { agent_id, result } => {
            match result {
                Ok(commands) => {
                    if let Some(agent) = app.agents.get_mut(&agent_id) {
                        agent.session.replace_available_commands(
                            commands,
                            CommandCatalogSource::CommandsList,
                        );
                        super::super::acp_handler::refresh_workflow_run_capabilities(agent);
                    }
                }
                Err(error) => tracing::warn!(%error, "command catalog refresh failed"),
            }
            vec![]
        }
        ...
        TaskResult::LogoutComplete { result } => match result {
            Ok(()) => {
            ...
            Err(error) => {
                app.show_toast(&format!("Could not log out. {error}"));
                vec![]
            }
        },
