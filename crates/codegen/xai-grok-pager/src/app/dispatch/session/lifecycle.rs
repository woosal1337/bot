// Modified by the Bot project on 2026-09-11: provider-aware session startup state.
...
pub(in crate::app::dispatch) fn dispatch_new_session_inner_with_id(
    ...
    let agent = AgentView::new(
        AgentSession {
            ...
            read_only_mode: matches!(app.current_ui.permission_mode.as_deref(), Some("read-only")),
    ...
    if let Some(agent) = app.agents.get_mut(&agent_id) {
        ...
        agent.mcp_init_progress = (crate::provider::active_provider()
            == crate::provider::ProviderId::Grok)
            .then(|| McpInitProgress {
                total: 0,
                connected: 0,
...
fn configure_agent_composer(app: &mut AppView, agent_id: AgentId) {
    ...
    agent.apply_app_scoped_gates(chat_mode, screen_mode);
    agent
        .prompt
        .slash_controller
        .registry_mut()
...
/// Callers must handle the view staying on Welcome while an acknowledgement is pending.
...
pub(in crate::app::dispatch) fn dispatch_new_worktree_session(
    ...
    };
    {
        ...
        agent.apply_app_scoped_gates(app.chat_mode, app.screen_mode);
        agent.chat_kind = chat_kind;
        agent.conversation_entry = chat_kind;
        #[cfg(feature = "local-workspace")]
        {
