// Modified by the Bot project on 2026-09-11: shared CLI flag classification.
...
pub(crate) fn screen_mode_relaunch_resume_hint(session_id: &str, want_minimal: bool) -> String {
    ...
    format!(
        "{GROK_SCREEN_MODE_ENV}={mode} bot --provider {} {flag} --resume {session_id}",
        provider.key()
    )
}
...
mod tests {
    ...
    fn failed_relaunch_hint_includes_screen_mode_env() {
        ...
        assert_eq!(
            ...
            "GROK_SCREEN_MODE=fullscreen bot --provider grok --fullscreen --resume abc-sid"
        );
        assert_eq!(
            screen_mode_relaunch_resume_hint("abc-sid", true),
            "GROK_SCREEN_MODE=minimal bot --provider grok --minimal --resume abc-sid"
        );
    }
