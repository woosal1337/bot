// Modified by the Bot project on 2026-09-12: Bot lifecycle and updater removal.
...
pub(crate) mod provider_relaunch;
...
async fn bounded_connect(
    ...
    let context = || startup_failure::Context {
        target,
        attempt,
        version: xai_grok_version::display_version_with_commit(xai_grok_version::full_version()),
...
pub async fn run(mut args: PagerArgs) -> anyhow::Result<()> {
    ...
    if let Some(value) = args.permission_mode_flag.as_deref() {
        let term = crate::provider::parse_permission_term(&requested_provider, value).ok_or_else(
            || {
                anyhow::anyhow!(
                    "Permission mode '{value}' is not available for {}",
                    provider_state.display_name()
                )
            },
        )?;
        args.permission_mode_flag = Some(term.kind.to_owned());
    }
    let is_grok_provider = requested_provider == crate::provider::ProviderId::Grok;
    let codex_executable = (requested_provider == crate::provider::ProviderId::Codex)
        .then(|| {
            provider_state
                .descriptor
                .executable_path()
    ...
    if is_grok_provider
        && let xai_grok_login::PreTuiLoginOutcome::SignedIn(auth) =
    ...
    let refreshed_auth = if is_grok_provider {
        tokio::time::timeout(
    ...
    let had_prefetch = is_grok_provider
    ...
    let LeaderMode {
        use_leader: configured_use_leader,
        policy_disable_reason,
    ...
    let use_leader = configured_use_leader && is_grok_provider;
    tracing::info!(
        use_leader,
        ?policy_disable_reason,
    ...
    let materialized = if is_grok_provider {
    ...
    let remote_permission_mode = is_grok_provider
        .then(|| {
            remote_settings
                .as_ref()
    ...
    let launch_auto = if is_grok_provider {
        xai_grok_shell::util::config::effective_auto_for_launch(
            args.yolo,
            args.permission_mode_flag.as_deref(),
            remote_permission_mode,
    ...
    let mut connect_flags = crate::acp::ConnectFlags {
        ...
        default_permission_mode: Some(launch_permission_mode.to_owned()),
        status_line: false,
    };
    ...
    let connect_result = bounded_connect(
        ...
        async {
            if let Some(executable) = codex_executable {
                crate::acp::connect_codex(&cancel, connect_flags, executable).await
            } else {
    ...
    let forced_exit_code = match &result {
        Ok(run_result) if run_result.relaunch.is_some() => None,
        Ok(_) => Some(0),
        Err(_) => Some(1),
    };
    ...
    match result {
        Ok(run_result) => {
            ...
            if let Some(relaunch) = run_result.relaunch.as_ref() {
                match relaunch {
                    app_view::ProcessRelaunch::ScreenMode(relaunch) => {
                    ...
                    app_view::ProcessRelaunch::Provider(provider) => {
                        if let Err(e) = provider_relaunch::exec_provider_relaunch(provider) {
                            tracing::error!(error = %e, "provider relaunch failed");
                            if terminal_reading {
                                let _ = writeln!(
                                    io::stderr(),
                                    "Failed to start {}: {e}\nRun `bot --provider {}` to retry.",
                                    provider.label(),
                                    provider.key()
                                );
                            }
...
mod tests {
    ...
    fn terminal_title_strips_control_characters() {
        assert_eq!(
            ...
            "evil]52;c;payloadtitle - bot"
        );
        assert_eq!(terminal_title_string("\x07\x1b\x00"), "bot");
        assert_eq!(terminal_title_string(""), "bot");
        assert_eq!(terminal_title_string("My chat"), "My chat - bot");
    }
    #[test]
    ...
    #[test]
    fn cli_command_name_is_bot() {
        use clap::CommandFactory;
        assert_eq!(PagerArgs::command().get_name(), "bot");
    }
    #[test]
    fn cli_help_output_header() {
        ...
        assert_eq!(
            first_5,
            vec![
                "Bot terminal agent interface",
                "",
                "Usage: bot [OPTIONS] [PROMPT] [COMMAND]",
                "",
                "Arguments:",
            ]
        );
    ...
    fn print_exit_resume_hint_writes_expected_lines() {
        ...
        assert_eq!(
            ...
            "\nResume this session with:\n  bot --provider grok --resume sess-abc\n"
        );
    }
    #[test]
    fn print_exit_resume_hint_includes_minimal_flag() {
        ...
        assert_eq!(
            String::from_utf8(buf).unwrap(),
            "\nResume this session with:\n  bot --provider grok --minimal --resume sess-abc\n"
        );
    }
    #[test]
    ...
    fn print_exit_resume_hint_truncates_summary_to_width() {
        ...
        assert!(out.contains(&format!("\n> {}…\n", "p".repeat(17))));
        assert!(out.contains(&format!("\n  {}…\n", "r".repeat(17))));
        assert!(out.contains("  bot --provider grok --resume sess-abc\n"));
    }
    #[test]
