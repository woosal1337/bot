...
#[derive(Debug, Clone)]
pub enum ProcessRelaunch {
    ScreenMode(ScreenModeRelaunch),
...
pub struct AppView {
    ...
    /// When set, the event loop should exit and re-exec after terminal restoration.
    /// Screen mode changes resume the active session. Provider changes start an isolated session.
    pub relaunch: Option<ProcessRelaunch>,
    ...
    /// Local document viewer overlay for the welcome screen.
...
impl AppView {
    ...
    /// True when the user should not see the prompt because ZDR access is blocked.
    ...
    pub fn new(
        ...
        Self {
            ...
            settings_registry: Arc::new(crate::settings::SettingsRegistry::bot()),
                ...
                match *active_view {
                    ...
                    ActiveView::Agent(id) => {
                        ...
                        if let Some(agent) = agents.get_mut(&id) {
                            ...
                            let banner_height = u16::from(has_mode_banner || show_session_tip);
    ...
    pub fn tick(&mut self) -> bool {
        ...
        for agent in self.agents.values_mut() {
            if agent
                .question_view
                .as_ref()
                .is_some_and(|view| view.auto_resolution_due(now))
            {
                let _ = agent.resolve_question_timeout();
                needs_redraw = true;
            }
            ...
            for child in agent.subagent_views.values_mut() {
                if child
                    ...
                    .is_some_and(|view| view.auto_resolution_due(now))
                {
                    let _ = child.resolve_question_timeout();
                    needs_redraw = true;
                }
    ...
    fn view_tick_demand(&self) -> TickDemand {
        ...
        if self.agents.values().any(|agent| {
            agent
                .question_view
                .as_ref()
                .is_some_and(|view| view.auto_resolution_pending())
