...
/// If a login or trust screen is still up, [`run`] drops the events rather than let that screen consume the keys.
...
pub(crate) struct RunResult {
    ...
    /// When set, the process should re-exec after terminal restoration.
    pub relaunch: Option<super::app_view::ProcessRelaunch>,
}
...
pub(crate) async fn run(
    ...
    let is_grok_provider = crate::provider::active_provider() == crate::provider::ProviderId::Grok;
    ...
    let display_mode = if !is_grok_provider {
    ...
    if app.ready_for_startup_typeahead() {
        // Composer is already the active input consumer.
        ...
        // A startup login, folder-trust, or ZDR screen is still up.
