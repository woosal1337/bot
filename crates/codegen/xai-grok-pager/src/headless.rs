// Modified by the Bot project on 2026-09-12: route headless prompts through the selected provider.
...
impl HeadlessOptions {
    fn validate_provider_options(&self, verbatim: bool) -> Result<bool> {
        let provider = crate::provider::require_ready(&self.provider)?;
        let is_grok = provider.id() == &crate::provider::ProviderId::Grok;
        if let Some(mode) = self.permission_mode_flag.as_deref()
            && crate::provider::parse_permission_term(provider.id(), mode).is_none()
        {
            let choices = crate::provider::permission_terms(provider.id())
                .iter()
                .map(|term| term.keyword)
                .collect::<Vec<_>>()
                .join(", ");
            anyhow::bail!(
                "Permission mode '{mode}' is not available for {}. Choose one of: {choices}.",
        ...
        let unsupported = [
            (verbatim, "--verbatim"),
            (self.json_schema.is_some(), "--json-schema"),
            (self.rules.is_some(), "--rules"),
            (self.system_prompt_override.is_some(), "--system-prompt"),
            (self.session_id.is_some(), "--session-id"),
            (self.worktree.is_some(), "--worktree"),
            (self.worktree_ref.is_some(), "--worktree-ref"),
            (self.restore_code, "--restore-code"),
            (self.agent.is_some(), "--agent"),
            (self.agents_json.is_some(), "--agents"),
            (self.cli_tools.is_some(), "--tools"),
            (self.cli_disallowed_tools.is_some(), "--disallowed-tools"),
            (self.disable_web_search, "--disable-web-search"),
            (!self.allow_rules.is_empty(), "--allow"),
            (!self.deny_rules.is_empty(), "--deny"),
            (self.max_turns.is_some(), "--max-turns"),
            (self.memory_flush, "--memory-flush"),
            (
                self.memory_enabled_override.is_some(),
                "--experimental-memory/--no-memory",
            ),
        ];
        if let Some((_, flag)) = unsupported.iter().find(|(set, _)| *set) {
            anyhow::bail!(
                "{flag} is not connected for {}. Remove this flag and retry.",
                provider.display_name()
            );
        }
        Ok(false)
...
async fn spawn_headless_agent(
...
) -> Result<crate::acp::AgentEndpoint> {
    let provider = crate::provider::require_ready(&options.provider)?;
    if provider.id() == &crate::provider::ProviderId::Codex {
        let executable = provider
            .descriptor
            .executable_path()
            .ok_or_else(|| anyhow::anyhow!("Codex CLI was not found"))?;
        let flags = crate::acp::ConnectFlags {
            default_yolo_mode: options.yolo,
            default_permission_mode: Some(if options.yolo {
                "always-approve".to_owned()
            } else {
                options
                    .permission_mode_flag
                    .as_deref()
                    .and_then(|value| crate::provider::parse_permission_term(provider.id(), value))
                    .map(|term| term.kind)
                    .unwrap_or("default")
                    .to_owned()
            }),
        ...
        return crate::acp::spawn_codex(executable.to_path_buf(), cancel, &flags).await;
    }
    ...
    let permission_term = options
        .permission_mode_flag
        .as_deref()
        .and_then(|value| crate::provider::parse_permission_term(provider.id(), value));
    let launch_yolo =
        options.yolo || permission_term.is_some_and(|term| term.kind == "always-approve");
    agent_config.default_yolo_mode = launch_yolo;
    agent_config.default_auto_mode = xai_grok_shell::util::config::effective_auto_for_launch(
        launch_yolo,
        permission_term.map(|term| term.kind),
        None,
    ...
    spawn_grok_shell(agent_config, cancel, memory_config)
        .await
        .map(crate::acp::AgentEndpoint::from)
}
...
pub async fn run_single_turn(
    ...
    let is_grok = options.validate_provider_options(verbatim)?;
    let provider = crate::provider::require_ready(&options.provider)?;
    crate::provider::set_active_provider(provider.id().clone());
    ...
    let spawned = match spawn_headless_agent(&options, &cancel).await {
        Ok(s) => s,
        Err(e) => {
    ...
    let (acp_tx, mut acp_rx, _agent_guard) = spawned.into_headless();
    ...
    let materialized = if is_grok {
        session_startup::materialize_startup_for_cwd(
        ...
        session_startup::materialize_provider_startup(intent)
    }
    .inspect_err(|_| {
        PendingStartup::finish_held(&mut pending_startup, crate::acp::StartupOutcome::Error);
    })?;
    ...
    // Seed the reducer's session context BEFORE applying model/effort so a later failure carries it.
    {
        ...
        emitter.begin_session(SessionContext {
            ...
            permission_mode,
            mcp_servers: if is_grok {
                mcp_server_names(&session_cwd)
            } else {
                Vec::new()
            },
    ...
    let session_models = if needs_fresh_catalog && is_grok {
            ...
            tokio::select! {
                ...
                signal = &mut interrupt, if !interrupted => {
                    ...
                    let cancel_request = acp::CancelNotification::new(session_id.clone())
                        .meta(serde_json::json!({"stopBackgroundTerminals": true}).as_object().cloned());
                    let _ = tokio::time::timeout(
                        Duration::from_secs(2),
                        acp_send(cancel_request, &acp_tx),
                    ).await;
                    interrupted = true;
                    interrupt_deadline = tokio::time::Instant::now() + Duration::from_secs(5);
                }
                _ = tokio::time::sleep_until(interrupt_deadline), if interrupted => {
                    break;
                }
                msg = acp_rx.recv() => {
    ...
    let outcome: Result<()> = match prompt_result {
        _ if interrupted => {
            emitter.on_error("Turn cancelled", Some("cancelled"));
