// Modified by the Bot project on 2026-09-13: use Bot diagnostic commands and messages.
...
pub const SSH_WRAP_FIX_COMMAND: &str = "bot doctor fix terminal.ssh-wrap";
pub const SSH_WRAP_ONE_OFF: &str = "bot wrap ssh <host>";

const MANAGED_NAMESPACE: &str = "grok doctor";
const SSH_WRAP_ALIAS_POSIX: &str = "alias ssh='bot wrap ssh'";
const SSH_WRAP_ALIAS_FISH: &str = "alias ssh 'bot wrap ssh'";
const TMUX_SCANNER_CAVEAT: &str = "Bot checks this file for direct global assignments of this option. Review sourced files, conditionals, plugins, and generated tmux setup yourself.";
    ...
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::UnknownId(id) => write!(
                formatter,
                "`{id}` is not an available Doctor fix. Run `bot doctor fix` to list available fixes."
            ),
            Self::PlatformUnsupported => write!(
                formatter,
                "Automatic SSH setup is not available on Windows. Run `{SSH_WRAP_ONE_OFF}` when needed."
            ),
            Self::HomeUnavailable => formatter.write_str("Bot could not find your home directory."),
            ...
            Self::ByobuConfigUnavailable => formatter.write_str(
                "Bot could not determine Byobu's effective config directory. Keep `BYOBU_CONFIG_DIR` set in this session, then run the fix again.",
            ),
            Self::UnsafeDirectory { label, path } => write!(
                formatter,
                "Bot refused unsafe {label} `{}`. Use a non-root absolute directory without control characters, `~`, `.` or `..` components.",
                path.display()
            ),
                    ...
                    || detail.contains("`ssh` fish function") =>
            {
                write!(
                    formatter,
                    "Bot found an existing SSH alias or function in {} and did not change it: {detail}",
                    path.display()
                )
            }
            Self::ExistingCustomization { path, detail } => write!(
                formatter,
                "Bot found an existing customization in {} and did not change it: {detail}",
                path.display()
            ),
            ...
            Self::PostconditionFailed => formatter
                .write_str("The configuration changed, but Bot could not verify the SSH alias."),
            Self::TmuxPostconditionFailed => formatter.write_str(
                "The configuration changed, but Bot could not verify the managed tmux option.",
            ),
        }
    }
}
...
const FIX_REGISTRY: &[FixSpec] = &[
    ...
    FixSpec {
        ...
        command: "bot doctor fix terminal.tmux-clipboard",
    ...
    FixSpec {
        ...
        command: "bot doctor fix terminal.dcs-passthrough",
    ...
    FixSpec {
        ...
        command: "bot doctor fix terminal.tmux-extended-keys",
    ...
    FixSpec {
        ...
        command: "bot doctor fix terminal.tmux-truecolor",
...
pub(crate) fn human_fix_command(id: DiagnosticId) -> Option<String> {
    fix_spec(id).map(|spec| format!("bot doctor fix {}", spec.handle))
}
    ...
    for (id, handle, availability) in fixes {
        ...
        match availability {
            AutomaticFixAvailability::Here => output.push_str(&format!(
                "    Run: bot doctor fix {handle}\n    In Bot: /doctor fix {handle}\n"
            )),
            AutomaticFixAvailability::RunLocally => output.push_str(&format!(
                "    On your local computer, run: bot doctor fix {handle}\n"
            )),
        }
    }
    output
...
pub(crate) fn format_fix_preview(plan: &FixPlan) -> String {
    ...
    match &change.backup_path_hint {
        Some(path) => {
            let _ = writeln!(
                output,
                "\nBackup will be saved to: {}\nIf that file exists, Bot will choose a unique name.",
                preview_path(path)
            );
        }
    ...
    match &plan.payload {
        FixPayload::SshWrap(_) => {
            output.push_str(
                "\nWhat this changes:\n  In new interactive shells, `ssh ...` runs as `bot wrap ssh ...`.\n",
            );
            let _ = writeln!(
                output,
...
fn plan_ssh_wrap(
    ...
    Ok(FixPlan {
        ...
        caveats: vec![
            ...
            "`bot wrap` starts the SSH process directly, so the alias does not loop.",
            "Bot checks this file for direct SSH aliases and functions. Review sourced files, plugins, and generated shell setup yourself.",
        ],
