...
fn canonical_and_short_ids_resolve_to_canonical_id() {
    ...
    assert_eq!(command, "bot doctor fix ssh-wrap");
    assert_eq!(
        resolve_fix_id(command.strip_prefix("bot doctor fix ").unwrap()).unwrap(),
        SSH_WRAP_ID
    );
...
fn bash_zsh_and_fish_plans_use_exact_paths_and_aliases() {
    ...
    for (shell, relative, alias) in [
        ("/bin/bash", ".bashrc", "alias ssh='bot wrap ssh'"),
        ("/bin/zsh", ".zshrc", "alias ssh='bot wrap ssh'"),
        (
            "/usr/local/bin/fish",
...
fn shell_aliases_expand_to_exact_argv_and_bypass_is_explicit() {
    ...
    if let Some(bash) = find_on_path("bash") {
        ...
        std::fs::write(&rc, "alias ssh='bot wrap ssh'\n").unwrap();
        let command = format!(
    ...
    shell
        .args([
            "--norc",
            "-ic",
            "alias ssh='bot wrap ssh'; command ssh host",
        ])
        .env("CAPTURE", &capture)
        .env(
            "PATH",
