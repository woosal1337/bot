...
pub enum WarningCategory {
    ...
    /// The session runs over SSH without `bot wrap` on the local end.
...
pub(crate) fn collect_startup_warnings_from(
    ...
    if ctx.brand == TerminalName::AppleTerminal && ctx.is_ssh {
        ...
        warning.note = Some(
            "Bot also saves each copy to the backup file shown in the copy message. To copy \
             directly, run `bot wrap ssh <host>` on your local computer or use a terminal that \
    ...
    if ctx.is_tmux_backed() && matches!(tmux.control_mode, probes::TmuxProbeResult::Available(true))
    {
        let message = match fullscreen_active {
            ...
            Some(false) => "Bot is using inline mode because tmux control mode limits fullscreen",
...
pub(crate) fn wezterm_kitty_keyboard_warning_from(
    ...
    if shape == WezTermShape::SshXtversion {
        ...
        warning.note = Some(
            "For this session, type `\\` and then press Enter. Bot can't negotiate the Kitty \
...
fn sandbox_profile_conflict_warning_from(conflicts: Vec<String>) -> Option<TerminalWarning> {
    ...
    Some(TerminalWarning {
        ...
        config_path: None,
        note: Some(format!(
            "Bot is using the user profile. Compare `.grok/sandbox.toml` with {}, then rename \
...
/// Pure SSH `bot wrap` recommendation: suggests launching the session through `bot wrap ssh <host>` on the user's
...
pub(crate) fn collect_notification_warnings_with_method(
        ...
        && ctx.brand == TerminalName::Unknown
    {
        let mut warning = TerminalWarning::new(
            ...
            "Bot is using the terminal bell because the terminal was not recognized",
            None,
            None,
        );
...
pub fn color_support_warning(
    ...
    if level == probes::RuntimeEvidence::Available(ColorLevel::None) {
        ...
        warning.note = Some("Unset `NO_COLOR`, then restart Bot.".to_string());
        return Some(warning);
    }
    ...
    if color_passthrough == TmuxColorPassthrough::Reduced {
        ...
        warning.note = Some(format!(
            "Run `tmux source-file {tmux_config_path}`, then detach and reattach: the server \
             ...
             If Bot still reports less than truecolor afterwards, also add `set -g \
    ...
    if is_tmux_backed {
        ...
        warning.note = Some(format!(
            "In the same tmux config, also add `set -g default-terminal \"tmux-256color\"`. Add \
             ...
             `tmux source-file {tmux_config_path}`, then detach and reattach, and restart Bot."
        ));
        return Some(warning);
    }
...
mod tests {
    ...
    // -- ssh_wrap_hint: `bot wrap ssh` recommendation ---------------------------
    ...
    fn ssh_wrap_hint_fires_over_plain_ssh() {
        ...
        assert_eq!(w.fix.as_deref(), Some("bot wrap ssh <host>"));
        assert!(
