...
fn tmux_config_and_reload_notes_output_is_stable() {
    ...
    assert_eq!(
        ...
        concat!(
            ...
            "      Automatic setup: `bot doctor fix tmux-clipboard`\n",
            ...
            "      Automatic setup: `bot doctor fix dcs-passthrough`\n",
            ...
            "      Automatic setup: `bot doctor fix tmux-extended-keys`\n",
...
fn limited_color_output_is_stable() {
    ...
    let terminal_theme_enabled = crate::theme::cache::terminal_theme_enabled();
    ...
    crate::theme::cache::set_terminal_theme_enabled(terminal_theme_enabled);
    ...
    assert_eq!(
        ...
        concat!(
            ...
            "      Note: Add this export to your shell startup file, such as `~/.zshrc` or `~/.bashrc`, then restart Bot.\n",
        )
    );
}
...
fn unwrapped_ssh_recommendation_with_no_issues_output_is_stable() {
    ...
    assert_eq!(
        ...
        concat!(
            ...
            "      Automatic setup: `bot doctor fix ssh-wrap`\n",
            "      One-off: `bot wrap ssh <host>`\n",
...
fn wezterm_xtversion_runtime_evidence_output_is_stable() {
    ...
    assert_eq!(
        ...
        concat!(
            ...
            "      Note: For this session, type `\\` and then press Enter. Bot can't negotiate the Kitty keyboard protocol over SSH yet. `enable_kitty_keyboard = true` applies only to local WezTerm sessions.\n",
        )
    );
}
...
fn runtime_startup_findings_are_visible_with_useful_doctor_content() {
    ...
    assert!(output.contains("Bot is using the terminal bell"));
...
fn runtime_findings_merge_before_single_formatter_orders_issues_before_recommendations() {
    ...
    let issue = output.find("Bot is using the terminal bell").unwrap();
