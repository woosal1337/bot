//! `bot completions <shell>`: generate shell completion scripts.
//!
...
pub fn run(shell: Shell) {
    // Ensure the script always uses the public "bot" name.
    let mut cmd = PagerArgs::command().name("bot");
    if shell != Shell::Zsh {
        generate(shell, &mut cmd, "bot", &mut std::io::stdout());
        return;
    }
    // zsh needs post-processing (see fix_zsh_root_prompt_positional).
    let mut buf = Vec::new();
    generate(shell, &mut cmd, "bot", &mut buf);
...
/// subcommand slot. Upstream bug: <https://github.com/clap-rs/clap/issues/6282>. `bot worktree <TAB>` then
...
fn fix_zsh_root_prompt_positional(script: &str) -> String {
    ...
    for (from, to) in [
        ...
        (
            r#"curcontext="${curcontext%:*:*}:bot-command-$line[2]:""#,
            r#"curcontext="${curcontext%:*:*}:bot-command-$line[1]:""#,
        ),
...
mod tests {
    ...
    fn zsh_script() -> String {
        let mut cmd = PagerArgs::command().name("bot");
        let mut buf = Vec::new();
        generate(Shell::Zsh, &mut cmd, "bot", &mut buf);
    ...
    // Dispatch happens on `$line[2]`, so `bot worktree <TAB>` re-offered every top-level command (upstream clap-rs/clap#6282)
    #[test]
