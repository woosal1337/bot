// Modified by the Bot project on 2026-09-14: use the Bot command identity.
//! `bot wrap` runs any command in a local PTY that forwards its clipboard.
//!
//! Generalizes the SSH wrapper: spawns an arbitrary command inside a local pseudo-terminal.
...
//! On Unix, a command a direct spawn cannot run (a single shell-quoted string `bot wrap "mycli ssh host"` or a shell alias) goes to `$SHELL -i -c`.
...
/// Run the `bot wrap` command. Otherwise the command is executed directly (no wrapping).
pub fn run(args: &WrapArgs) -> Result<()> {
    // `command` is `required` in clap, so it always has at least one element.
    let program = args
        .command
        .first()
        .ok_or_else(|| anyhow::anyhow!("bot wrap: no command given"))?;
    ...
    if should_wrap() {
        match crate::pty_wrap::run_wrapped_command(&wrapped.program, &wrapped.args) {
            ...
            Err(e) => {
                ...
                eprintln!("bot wrap: wrapped mode failed, running without PTY wrapping: {e}");
...
/// The program and argv `bot wrap` will actually spawn.
#[derive(Clone)]
struct SpawnPlan {
    program: String,
    args: Vec<String>,
