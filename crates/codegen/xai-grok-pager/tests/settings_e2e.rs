// Modified by the Bot project on 2026-09-13: cover the provider-neutral settings interface.
...
/// Canonical choices follow the active provider.
#[test]
fn pr6_permission_mode_choices_use_canonical_strings() {
    ...
    let expected = if xai_grok_pager::provider::active_provider()
        == xai_grok_pager::provider::ProviderId::Codex
    {
        vec!["ask", "auto", "always-approve", "read-only"]
    } else {
        vec!["ask", "auto", "always-approve"]
    };
...
/// Picking "Auto" dispatches `SetPermissionMode(Auto)`.
#[test]
fn pr11_picker_commit_for_auto_dispatches_set_permission_mode_auto() {
    use xai_grok_pager::app::actions::PermissionModeKind;
    let reg = SettingsRegistry::defaults();
    let meta = reg.find("permission_mode").unwrap();
    let choices = match &meta.kind {
    ...
    let auto_idx = choices
        .iter()
        .position(|c| c.canonical == "auto")
        .expect("permission_mode must include the 'auto' choice");
    let initial_idx = choices
        .iter()
        .position(|c| c.canonical == "ask")
        .expect("'ask' canonical must be present");
    assert_ne!(
        auto_idx, initial_idx,
        "test invariant: 'auto' must be a distinct choice from 'ask'"
    );
    ...
    let steps = auto_idx as isize - initial_idx as isize;
    let nav_key = if steps > 0 {
        KeyCode::Down
    } else {
        KeyCode::Up
    ...
    let outcome = handle_settings_key(&mut s, &press(KeyCode::Enter));
    match outcome {
        SettingsKeyOutcome::Action(Action::SetPermissionMode(PermissionModeKind::Auto)) => {}
        other => {
            panic!("Enter on 'auto' must commit Action::SetPermissionMode(Auto), got {other:?}")
        }
    }
    assert!(
        matches!(s.mode(), SettingsModalMode::Browse),
...
fn pr11_permission_mode_kind_canonical_strings_match_choices_catalog() {
    ...
    for canonical in &catalog_canonicals {
        assert!(
            PermissionModeKind::from_canonical(canonical).is_some(),
            "catalog contains unsupported permission mode `{canonical}`",
        );
    }
    let expected: std::collections::HashSet<&str> = if xai_grok_pager::provider::active_provider()
        == xai_grok_pager::provider::ProviderId::Codex
    {
        ["ask", "auto", "always-approve", "read-only"]
            .into_iter()
            .collect()
    } else {
        ["ask", "auto", "always-approve"].into_iter().collect()
    };
    assert_eq!(catalog_canonicals, expected);
}
...
fn reset_overlay_dims_all_rows_except_target() {
    ...
    for label in ["y reset", "n cancel"] {
        let action_y = find_row_y(label).unwrap_or_else(|| panic!("{label} must be visible"));
        let width = label.chars().count() as u16;
        let action_x = (area.x..area.x + area.width - width)
            .find(|&start| {
                (0..width)
                    .map(|dx| buf.cell((start + dx, action_y)).unwrap().symbol())
                    .collect::<String>()
                    == label
            })
            .unwrap_or_else(|| panic!("{label} cells must be visible"));
        assert!(
            !has_dim(action_x, action_y),
            "the {label} key must stay at full intensity"
        );
    }
}

/// The settings modal renders a one-line agent tip footer at the bottom of the content area. It shows in Browse,
...
fn docs_footer_renders_for_browse_and_picker() {
    ...
    for fixture_label in ["browse", "picker"] {
        ...
        assert!(
            all_text.contains("Ask your agent"),
            "[{fixture_label}] agent tip must appear in the rendered modal:\n\
             {all_text}"
        );
        assert!(
            all_text.contains("change the theme"),
...
fn expanded_description_wraps_to_modal_width() {
    ...
    assert!(rendered.contains("Ask prompts"));
    assert!(rendered.contains("Auto uses"));
...
/// Mirrors the theme and permission-mode pickers.
#[test]
...
/// Mirror of `pr6_permission_mode_picker_nav_does_not_dispatch_preview`.
#[test]
...
/// Mirrors the permission-mode escape path.
...
// Mirrors the permission-mode mouse tests. Every keyboard interaction has a mouse equivalent.
...
// ---------------------------------------------------------------------------
// CLI setting: show_tips (SHELL Bool, restart_required)
// ---------------------------------------------------------------------------
...
fn pr13_cli_batch_all_settings_are_restart_required() {
    ...
    let meta = reg
        .find("show_tips")
        .unwrap_or_else(|| panic!("registry must contain `show_tips` (PR 13)"));
    assert!(
        meta.restart_required,
        "PR-13 setting `show_tips` must have restart_required: true \
...
fn pr13_cli_batch_defaults_roundtrip_via_current_value_for() {
    ...
    let value = current_value_for("show_tips", &ui, &pager)
        .unwrap_or_else(|| panic!("current_value_for(`show_tips`) must resolve"));
    assert_eq!(
        value,
        SettingValue::Bool(true),
        "PR 13: `show_tips` defaults to true (matches the consumer's \
