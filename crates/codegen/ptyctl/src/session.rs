// Modified by the Bot project on 2026-09-15: use deterministic PTY test shutdown.
...
pub(crate) mod tests {
    ...
    async fn wait_text_matches_delayed_output() {
        let session = start_session(vec![
            "/bin/sh".into(),
            "-c".into(),
            "sleep 0.3; echo READY; read -r line".into(),
        ])
        .await;
        ...
        // Complete the trailing read so the child exits.
        shutdown(&session, "done<CR>").await;
    }
