// Modified by the Bot project on 2026-09-13: xAI feedback tool removal.
...
mod tests {
    ...
    #[test]
    fn built_in_toolsets_omit_removed_feedback_tool() {
        for (name, config) in all_toolset_presets() {
            assert!(
                config.tools.iter().all(|tool| {
                    tool.id != "GrokBuild:send_feedback"
                        && tool.name_override.as_deref() != Some("send_feedback")
                }),
                "preset `{name}` must not expose send_feedback"
            );
        }
    }
    #[test]
