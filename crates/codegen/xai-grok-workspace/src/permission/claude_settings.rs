// Modified by the Bot project on 2026-09-15: isolate tests from host Claude settings.
...
fn global_claude_settings_paths() -> Vec<PathBuf> {
    ...
    #[cfg(test)]
    let home = std::env::var_os("XAI_WORKSPACE_TEST_CLAUDE_HOME").map(PathBuf::from);
    #[cfg(not(test))]
