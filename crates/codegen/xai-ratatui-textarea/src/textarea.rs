...
impl TextArea {
    pub fn render_masked(
        ...
        let (content_width, needs_scrollbar) = self.content_width(area.width, area.height);
        ...
        for (row, range) in lines[start..end].iter().enumerate() {
            let width = self
                .display_width_of_range(range.start, range.end)
                .min(content_area.width as usize);
            buf.set_string(
                content_area.x,
                content_area.y + row as u16,
                mask_character.to_string().repeat(width),
                Style::default(),
            );
        }
        if needs_scrollbar {
