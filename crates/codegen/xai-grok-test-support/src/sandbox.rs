// Modified by the Bot project on 2026-09-15: redact host paths and select Grok in test environments.
...
impl TestSandbox {
    ...
    pub fn diagnostic_summary(&self) -> String {
        ...
        for (key, value) in &self.env {
            ...
            let display = if is_secret_key(&key) || is_host_path_key(&key) {
                REDACTED.to_owned()
...
fn is_host_path_key(key: &str) -> bool {
    matches!(
        key.to_ascii_uppercase().as_str(),
        "PATH" | "LD_LIBRARY_PATH" | "DYLD_LIBRARY_PATH" | "GIT_BIN_PATH" | "GIT_EXEC_PATH"
    )
}
...
mod tests {
    ...
    fn baseline_is_hermetic_and_network_quiet() {
        ...
        assert_eq!(
            env_value(&sandbox, "BOT_PROVIDER").as_deref(),
            Some(OsStr::new("grok"))
        );
        assert_eq!(
    ...
    fn diagnostics_show_only_sanitized_loopback_urls() {
        ...
        assert!(summary.contains("PATH=<redacted>"), "{summary}");
