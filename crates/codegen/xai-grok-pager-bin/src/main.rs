// Modified by the Bot project on 2026-09-12: command identity and TUI updater removal.
#![allow(
    unused_imports,
    unused_variables,
    unused_mut,
...
use xai_grok_telemetry::process_info::{Entrypoint, Interactivity, ProcessIdentity, set_identity};
...
fn print_serve_startup_info(bind_addr: SocketAddr, secret: &str) {
    eprintln!();
    eprintln!("   Bot agent server starting...");
    eprintln!();
...
async fn run_setup_command(json: bool) {
    ...
    match managed_config::run_setup().await {
        ...
        SetupOutcome::Staged => {
            eprintln!(
                "Managed configuration update verified; it takes effect the next time Bot starts."
            );
        }
...
fn main() {
    ...
    if let Err(e) = xai_grok_config::validate_requirements() {
        eprintln!("Couldn't start Bot: {e}");
        eprintln!();
        eprintln!(
            "Update Bot to a version the policy allows, or ask your administrator \
    ...
    if xai_grok_shell::util::config::load_crash_handler_enabled_sync() {
        ...
        if let Some(report) = xai_crash_handler::check_previous_crash(&crash_dir) {
            eprintln!("Bot crashed during your last session.");
...
async fn async_main(mut args: PagerArgs) -> Result<()> {
    ...
    let uses_grok_session_store = args.provider.eq_ignore_ascii_case("grok");
    if !uses_grok_session_store
    ...
    {
        anyhow::bail!(
            "This command uses Grok. Select --provider grok, or use the native {} CLI.",
            args.provider
        );
    }
    if uses_grok_session_store {
    ...
    let saved_profile = uses_grok_session_store
...
mod tests {
    ...
    #[test]
    fn version_output_writer_uses_bot_identity() {
        xai_grok_version::set_full_version(env!("VERSION_WITH_COMMIT"));
