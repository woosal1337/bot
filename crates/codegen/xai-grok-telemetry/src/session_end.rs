// Modified by the Bot project on 2026-09-13: remove xAI feedback shutdown timing.
...
//! Wrapper phases (`Memory`, `Hooks`, `Workflows`, `SessionFlush`) are span-only aggregates of their leaves.
...
fn phase_event_slot(
    ...
    match phase {
        ...
        Phase::SessionFlush | Phase::Workflows | Phase::Hooks | Phase::Memory => None,
    }
}
...
pub fn timed(timer: &SharedSessionEndTimer, phase: Phase) -> TimedPhase {
    TimedPhase {
        region: span(phase),
        timer: Arc::clone(timer),
        phase,
...
mod tests {
    ...
    fn write_event_phases_maps_every_leaf_and_skips_wrappers() {
        ...
        timer.record(Phase::BackgroundTasksSave, Duration::from_millis(7));
        ...
        assert_eq!(event.background_tasks_save_ms, Some(7));
    }
}
