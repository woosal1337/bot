...
// Modified by the Bot project on 2026-09-15: keep the Loom harness compatible with all-feature builds.

#[cfg(all(test, feature = "loom", not(debug_assertions)))]
#[path = "flock_wait_loom_tests.rs"]
mod loom_tests;

#[cfg(all(test, any(not(feature = "loom"), debug_assertions)))]
...
#[cfg(not(all(test, feature = "loom", not(debug_assertions))))]
