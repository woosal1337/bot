...
/// The per-client `record_401_attribution` wrappers resolve their bearer and call this with the right `(kind, op)`. `sent_bearer` may be a full bearer or a 12-char prefix. The sampler-side [`Auth401AttributionCallback`] boundary passes a prefix; the sampler scrubs before crossing the crate boundary.
